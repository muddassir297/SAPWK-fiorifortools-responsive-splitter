 ...
 */
sap.ui.define(['jquery.sap.global', './library', 'sap/ui/core/Control', 'sap/ui/core/ResizeHandler', 'sap/ui/Device'],
	function(jQuery, library, Control, ResizeHandler, Device) {
	"use strict";
	...
	InteractiveDonutChart.prototype._onResize = function() {
		...
		// chart visibility
		if (iCurrentControlWidth < InteractiveDonutChart.CHART_WIDTH_MINVALUE || iCurrentControlHeight < InteractiveDonutChart.CHART_HEIGHT_MINVALUE || iInteractionAreaHeight < InteractiveDonutChart.AREA_HEIGHT_MINVALUE) {
		...
		if (iCurrentControlWidth < InteractiveDonutChart.CHART_WIDTH_HIDEDONUT_MINVALUE) {
			...
			if ($DonutContainer.innerWidth() < $DonutContainer.innerHeight()) {
				...
				$This.find(".sapSuiteIDCChartSVG").css("height", "100%").css("width", $DonutContainer.innerHeight() + "px");
			}
