...
sap.ui.define(["jquery.sap.global", './library', "sap/ui/core/theming/Parameters"],
	...
	InteractiveDonutChartRenderer.render = function(oRm, oControl) {
		...
		var iSegmentsNum = Math.min(oControl.getDisplayedSegments(), this._aSegments.length);
	...
	InteractiveDonutChartRenderer._renderDonut = function(oRm, oControl, segmentsNum) {
		...
		var bCompact = oControl._bCompact;
	...
	InteractiveDonutChartRenderer._getSegmentColor = function(position, segmentsNum) {
		var fWeight = 1 - (segmentsNum - position) / segmentsNum,
			sColor = Parameters.get("_sap_suite_idc_color") || "white"; //fallback to white
	...
	InteractiveDonutChartRenderer._mixColors = function(color1, color2, weight) {
		...
		var aColor1 = HexToRgb(color1);
		var aColor2 = HexToRgb(color2);
		...
		return "#" + IntToHex(sColorMixR) + IntToHex(sColorMixG) + IntToHex(sColorMixB);
