...
sap.ui.define(['jquery.sap.global'],
	...
	BulletMicroChartRenderer.render = function(oRm, oControl) {
		...
		if (bIsActualSet) {
			...
			oRm.addClass(jQuery.sap.encodeHTML(sSize));
			oRm.writeClasses();
			oRm.addStyle(jQuery.sap.encodeHTML(sOrientation), jQuery.sap.encodeHTML(parseFloat(oChartData.actualValuePct) + parseFloat(1) + "%"));
			...
			if (sMode == "sapSuiteBMCModeTypeActual" && oChartData.actualValuePct !== 0) {
				oRm.write("<div");
				...
				oRm.addStyle(jQuery.sap.encodeHTML(sOrientation), jQuery.sap.encodeHTML(1 + Math.min(oChartData.actualValuePct, oChartData.targetValuePct) + "%"));
				oRm.writeStyles();
				oRm.writeAttribute("id", oControl.getId() + "-bc-bar-value");
				oRm.write("></div>");
			}
		...
		if (oControl._isTargetValueSet) {
			...
			oRm.addClass(jQuery.sap.encodeHTML(sSize));
			oRm.writeClasses();
			oRm.addStyle(jQuery.sap.encodeHTML(sOrientation), jQuery.sap.encodeHTML(parseFloat(oChartData.targetValuePct).toFixed(2) + "%"));
			oRm.writeStyles();
	...
	BulletMicroChartRenderer.renderThreshold = function(oRm, oControl, oThreshold) {
		...
		if (sColor === "sapSuiteBMCSemanticColor" + sap.m.ValueColor.Error) {
			...
			oRm.writeClasses();
			oRm.addStyle(jQuery.sap.encodeHTML(sOrientation), jQuery.sap.encodeHTML(fValuePct + "%"));
			oRm.writeStyles();
			oRm.write("></div>");
		}
		oRm.write("<div");
