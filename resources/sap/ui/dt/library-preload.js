...
a.prototype.init=function(){this._fnFireModified=function(){this.fireModified();}.bind(this);};
...
a.prototype.init=function(){this._fnFireResizeDomChanged=function(){this.fireDomChanged({type:"resize"});}.bind(this);this._onScroll=this._fireDomChangeOnScroll.bind(this);this._startMutationObserver();this._startResizeObserver();this._startScrollObserver();};
a.prototype.exit=function(){this._stopMutationObserver();this._stopResizeObserver();this._stopScrollObserver();};
a.prototype._startMutationObserver=function(){if(this._oMutationObserver){return;}var a=window.MutationObserver||window.WebKitMutationObserver;if(a){this._oMutationObserver=new a(function(m){var t=[];var e=[];m.forEach(function(o){var T=o.target;if(o.type==="characterData"){T=o.target.parentNode;}if(!O.isInOverlayContainer(T)){t.push(T);var b=O.getClosestOverlayForNode(T);var s=b?b.getElementInstance().getId():undefined;if(s){e.push(s);}}});if(t.length){this.fireDomChanged({type:"mutation",elementIds:e,targetNodes:t});}}.bind(this));this._oMutationObserver.observe(window.document,{childList:true,subtree:true,attributes:true,attributeFilter:["style","class","width","height","border"],characterData:true});}else{q.sap.log.error("Mutation Observer is not available");}};
...
a.getParentInformation=function(e){var p=e.getParentElementOverlay();if(p){var P=p.getElementInstance();var s=e.getParentAggregationOverlay().getAggregationName();var c=E.getAggregation(P,s);var o=e.getElementInstance();var i=c.indexOf(o);var b=e.getPublicParentElementOverlay();return{publicParent:b.getElementInstance(),publicAggregation:e.getPublicParentAggregationOverlay().getAggregationName(),parent:P,aggregation:s,index:i};}else{return{publicParent:null,publicAggregation:"",parent:null,aggregation:"",index:-1};}};
...
a.iterateOverlayElementTree=function(e,c){c(e);e.getAggregationOverlays().forEach(function(A){A.getChildren().forEach(function(C){this.iterateOverlayElementTree(C,c);},this);},this);};
a.iterateOverlayTree=function(o,c){c(o);o.getChildren().forEach(function(C){this.iterateOverlayTree(C,c);},this);};
...
var a=P.extend("sap.ui.dt.plugin.ContextMenu",{metadata:{library:"sap.ui.dt",properties:{contextElement:{type:"object"}},associations:{},events:{openedContextMenu:{},closedContextMenu:{}}}});
...
a.prototype._onItemSelected=function(e){var i=e.getParameter("item").data("id");this._aMenuItems.some(function(I){if(i===I.id){var d=this.getDesignTime();var s=d.getSelection();I.handler(s);return true;}},this);};
...
var C=D.extend("sap.ui.dt.plugin.ControlDragDrop",{metadata:{library:"sap.ui.dt",properties:{draggableTypes:{type:"string[]",defaultValue:["sap.ui.core.Element"]},elementMover:{type:"sap.ui.dt.plugin.ElementMover"}},associations:{}}});
...
C.prototype.onDragEnter=function(t){var o=this.getDraggedOverlay();if(t.getElementInstance()!==o.getElementInstance()&&t!==this._oPreviousTarget){this.getElementMover().repositionOn(o,t);}this._oPreviousTarget=t;};
...
var C=P.extend("sap.ui.dt.plugin.CutPaste",{metadata:{library:"sap.ui.dt",properties:{movableTypes:{type:"string[]",defaultValue:["sap.ui.core.Element"]},elementMover:{type:"sap.ui.dt.plugin.ElementMover"}},associations:{}}});
...
C.prototype._onKeyDown=function(e){var o=sap.ui.getCore().byId(e.currentTarget.id);var c=sap.ui.Device.os.macintosh?e.metaKey:e.ctrlKey;if((e.keyCode===jQuery.sap.KeyCodes.X)&&(e.shiftKey===false)&&(e.altKey===false)&&(c===true)){this.cut(o);e.stopPropagation();}else if((e.keyCode===jQuery.sap.KeyCodes.V)&&(e.shiftKey===false)&&(e.altKey===false)&&(c===true)){this.paste(o);e.stopPropagation();}else if(e.keyCode===jQuery.sap.KeyCodes.ESCAPE){this.stopCutAndPaste();e.stopPropagation();}};
...
C.prototype._executePaste=function(t){var c=this.getElementMover().getMovedOverlay();if(!c){return false;}if(!this._isForSameElement(c,t)){var T=this._getTargetZoneAggregation(t);if(T){this.getElementMover().insertInto(c,T);return true;}else if(O.isInTargetZoneAggregation(t)){this.getElementMover().repositionOn(c,t);return true;}else{return false;}}setTimeout(function(){c.focus();},0);};
...
b.prototype.repositionOn=function(m,t){var o=m.getElementInstance();var T=O.getParentInformation(t);var A;var f=m.getFirstHiddenAggregationOverlay();var F=f.getElementInstance();var p=m.getPublicParentElementOverlay();if(f&&p){var s=f.getAggregationName();A=p.getDesignTimeMetadata().getAggregation(s);}if(T.index!==-1){if(A&&A.beforeMove){A.beforeMove(F,o);}E.insertAggregation(T.parent,T.aggregation,o,T.index);if(A&&A.afterMove){A.afterMove(F,o);}}};
b.prototype.insertInto=function(m,t){var o=m.getElementInstance();var T=t.getElementInstance();var A;var f=m.getFirstHiddenAggregationOverlay();var F=f.getElementInstance();var p=m.getPublicParentElementOverlay();if(f&&p){var s=f.getAggregationName();A=p.getDesignTimeMetadata().getAggregation(s);}var S=m.getParent();if(t!==S){if(A&&A.beforeMove){A.beforeMove(F,o);}var c=t.getAggregationName();E.addAggregation(T,c,o);if(A&&A.afterMove){A.afterMove(F,o);}}};
...
T.prototype._removeTabIndex=function(){var d=this.getDesignTime();var r=d.getRootElements();r.forEach(function(R){var o=sap.ui.getCore().byId(R).getDomRef();q(o).find(":focusable:not([tabIndex=-1], #overlay-container *)").each(function(i,n){n.setAttribute("data-sap-ui-dt-tabindex",n.tabIndex);n.setAttribute("tabIndex",-1);});});};
...
A.prototype.getMoveAction=function(m){var d=this.getData();if(d.actions&&d.actions.move){var M=d.actions.move;if(typeof(M)==="function"){return M.apply(null,arguments);}return M;}};
return A;},true);
sap.ui.predefine('sap/ui/dt/AggregationOverlay',['jquery.sap.global','sap/ui/dt/Overlay'],function(q,O){"use strict";
...
A.prototype.getAssociatedDomRef=function(){var e=this.getElementInstance();var a=this.getAggregationName();var d=this.getDesignTimeMetadata();return d.getAssociatedDomRef(e,d.getDomRef(),a);};
...
D.prototype._createChildOverlaysForAggregation=function(e,A){var o=e.getAggregationOverlay(A);var d=e.getElementInstance();var C=b.getAggregation(d,A);b.iterateOverElements(C,function(f){this._createElementOverlay(f,o.isInHiddenTree());}.bind(this));};
...
D.prototype._onElementModified=function(e){var p=e.getParameters();if(p.type==="addOrSetAggregation"||p.type==="insertAggregation"){this._onElementOverlayAddAggregation(p.value,p.target,p.name);}else if(p.type==="setParent"){setTimeout(function(){if(!this.bIsDestroyed){this._checkIfOverlayShouldBeDestroyed(p.target,p.value);}}.bind(this),0);}};
...
D.prototype._iterateRootElements=function(s,o){var r=this.getRootElements();r.forEach(function(R){var d=b.getElementInstance(R);s.call(o||this,d);},this);};
...
var a=M.extend("sap.ui.dt.DesignTimeMetadata",{metadata:{library:"sap.ui.dt",properties:{data:{type:"object"},libraryName:"string"}}});
a.prototype.setData=function(d){var m=q.extend(true,this.getDefaultData(),d||{});this.setProperty("data",m);return this;};
a.prototype.getData=function(){var d=this.getProperty("data");if(!d){this.setData({});d=this.getProperty("data");}return d;};
a.prototype.getDefaultData=function(){return{ignore:false,domRef:undefined,cloneDomRef:false};};
a.prototype.isIgnored=function(){return this.getData().ignore;};
a.prototype.getCloneDomRef=function(){return this.getData().cloneDomRef;};
...
a.prototype.getLibraryText=function(k,A){var l=sap.ui.getCore().getLibraryResourceBundle(this.getLibraryName());return l.getText(k,A);};
return a;},true);
...
E.prototype.createAggregationDesignTimeMetadata=function(a){var d=this.getAggregation(a);return new A({libraryName:this.getLibraryName(),data:d});};
E.prototype.getAggregations=function(){return this.getData().aggregations;};
E.prototype.getRelevantContainer=function(e){var g=this.getData().getRelevantContainer;if(!g||typeof g!=="function"){return e.getParent();}return g(e);};
E.prototype.getAggregationAction=function(a,e){var v;var o=this.getAggregations();var b=[];for(var s in o){if(o[s].actions&&o[s].actions[a]){v=o[s].actions[a];if(typeof v==="function"){v=v.call(null,e);}else if(typeof(v)==="string"){v={changeType:v};}if(v){v.aggregation=s;}b.push(v);}}return b;};
E.prototype._getText=function(n){if(typeof n==="function"){return n();}else{return this.getLibraryText(n);}};
E.prototype.getAggregationDescription=function(a,e){var c=this.getAggregation(a).childNames;if(typeof c==="function"){c=c.call(null,e);}if(c){return{singular:this._getText(c.singular),plural:this._getText(c.plural)};}};
E.prototype.getName=function(e){var n=this.getData().name;if(typeof n==="function"){n=n.call(null,e);}if(n){return{singular:this._getText(n.singular),plural:this._getText(n.plural)};}};
return E;},true);
...
var e=O.extend("sap.ui.dt.ElementOverlay",{metadata:{library:"sap.ui.dt",associations:{editableByPlugins:{type:"any[]",multiple:true,singularName:"editableByPlugin"}},properties:{selected:{type:"boolean",defaultValue:false},selectable:{type:"boolean",defaultValue:false},movable:{type:"boolean",defaultValue:false},editable:{type:"boolean",defaultValue:false}},aggregations:{aggregationOverlays:{type:"sap.ui.dt.AggregationOverlay",multiple:true},designTimeMetadata:{type:"sap.ui.dt.ElementDesignTimeMetadata",altTypes:["object"],multiple:false}},events:{selectionChange:{parameters:{selected:{type:"boolean"}}},movableChange:{parameters:{movable:{type:"boolean"}}},selectableChange:{parameters:{selectable:{type:"boolean"}}},editableChange:{parameters:{editable:{type:"boolean"}}},elementModified:{parameters:{type:"string",name:"string",value:"any",oldValue:"any",target:"sap.ui.core.Element"}},requestElementOverlaysForAggregation:{parameters:{name:{type:"string"}}}}}});
...
e.prototype.setDesignTimeMetadata=function(v){var o;if(v instanceof E){o=v;}else{o=new E({libraryName:this.getElementInstance().getMetadata().getLibraryName(),data:v});}var r=this.setAggregation("designTimeMetadata",o);if(this.getElementInstance()){this._renderAndCreateAggregation();}return r;};
...
e.prototype.getAssociatedDomRef=function(){var o=c.getDomRef(this.getElementInstance());if(!o){var f=this.getDesignTimeMetadata();if(!f){return;}var g=f.getDomRef();if(typeof g==="function"){o=g(this.getElementInstance());}}if(o){return jQuery(o);}};
...
e.prototype.setSelected=function(s,S){s=!!s;if(this.isSelectable()&&s!==this.isSelected()){this.setProperty("selected",s);this.toggleStyleClass("sapUiDtOverlaySelected",s);if(!S){this.fireSelectionChange({selected:s});}}return this;};
e.prototype.setMovable=function(m){m=!!m;if(this.getMovable()!==m){this.toggleStyleClass("sapUiDtOverlayMovable",m);this.setProperty("movable",m);this.fireMovableChange({movable:m});}return this;};
...
e.prototype.sync=function(){if(this.isVisible()){var f=this.getAggregationOverlays();f.forEach(function(o){this._syncAggregationOverlay(o);},this);}};
e.prototype._createAggregationOverlay=function(s,i){var o=this.getDesignTimeMetadata().createAggregationDesignTimeMetadata(s);var f=new a({aggregationName:s,element:this.getElementInstance(),designTimeMetadata:o,inHiddenTree:i});this._mAggregationOverlays[s]=f;this.addAggregation("aggregationOverlays",f);this._syncAggregationOverlay(f);f.attachVisibleChanged(this._onAggregationVisibleChanged,this);return f;};
e.prototype._createAggregationOverlays=function(){this._mAggregationOverlays={};var o=this.getElementInstance();var f=this.getDesignTimeMetadata();var m={};c.iterateOverAllPublicAggregations(o,function(i,j){var s=i.name;m[s]=true;this._createAggregationOverlay(s,this.isInHiddenTree());}.bind(this));var g=f.getAggregations();var h=Object.keys(g);h.forEach(function(s){var i=g[s];if(i.ignore===false&&!m[s]){var I=i.inHiddenTree;this._createAggregationOverlay(s,I);}},this);this.sync();};
...
e.prototype.getPublicParentAggregationOverlay=function(){var o=this.getParentAggregationOverlay();var p=this.getParentElementOverlay();while(p&&c.isInstanceOf(p,"sap.ui.dt.ElementOverlay")&&p.isInHiddenTree()){o=p.getParentAggregationOverlay();p=p.getParentElementOverlay();}return o;};
e.prototype.getPublicParentElementOverlay=function(){var p=this.getPublicParentAggregationOverlay();if(p){return p.getParent();}};
return e;},true);
...
E.isValidForAggregation=function(p,a,e){var A=p.getMetadata().getAggregation(a);if(this.hasAncestor(p,e)){return false;}if(A){var t=A.type;if(A.multiple===false&&this.getAggregation(p,a)){return false;}return this.isInstanceOf(e,t)||this.hasInterface(e,t);}};
...
b.getOverlayContainer=function(){if(!a){a=q.sap.byId(o);if(!a.length){a=q("<div id='"+o+"'></div>").appendTo("body");}}return a.get(0);};
...
b.prototype.setFocusable=function(f){f=!!f;if(this.isFocusable()!==f){this.setProperty("focusable",f);this.toggleStyleClass("sapUiDtOverlayFocusable",f);this.fireFocusableChange({focusable:f});}return this;};
...
b.prototype._onSyncScrollWithDomRef=function(e){window.clearTimeout(this._iSyncScrollWithDomRef);this._iSyncScrollWithDomRef=window.setTimeout(function(){this._syncScrollWithDomRef();delete this._iSyncScrollWithDomRef;}.bind(this),0);};
...
b.prototype.setInHiddenTree=function(i){i=!!i;if(i!==this.isInHiddenTree()){this.toggleStyleClass("sapUiDtOverlayInHiddenTree",i);this.setProperty("inHiddenTree",i);}return this;};
...
a.prototype._preventScrollOnTouch=function(e){if(p){e.preventDefault();}};
a.prototype.init=function(){P.prototype.init.apply(this,arguments);document.addEventListener('touchmove',this._preventScrollOnTouch,true);this._mElementOverlayDelegate={"onAfterRendering":this._checkMovable};this._mAggregationOverlayDelegate={"onAfterRendering":this._attachDragScrollHandler,"onBeforeRendering":this._removeDragScrollHandler};this._dragScrollHandler=this._dragScroll.bind(this);this._dragLeaveHandler=this._dragLeave.bind(this);this._mScrollIntervals={};};
a.prototype.exit=function(){P.prototype.exit.apply(this,arguments);document.removeEventListener('touchmove',this._preventScrollOnTouch);delete this._mElementOverlayDelegate;delete this._mAggregationOverlayDelegate;delete this._dragScrollHandler;};
a.prototype.registerElementOverlay=function(e){e.addEventDelegate(this._mElementOverlayDelegate,this);e.attachEvent("movableChange",this._onMovableChange,this);if(e.isMovable()){this._attachDragEvents(e);}e.attachBrowserEvent("dragover",this._onDragOver,this);e.attachBrowserEvent("dragenter",this._onDragEnter,this);e.attachBrowserEvent("dragleave",this._onDragLeave,this);};
...
a.prototype.deregisterElementOverlay=function(e){e.removeEventDelegate(this._mElementOverlayDelegate,this);e.detachEvent("movableChange",this._onMovableChange,this);this._detachDragEvents(e);e.detachBrowserEvent("dragover",this._onDragOver,this);e.detachBrowserEvent("dragenter",this._onDragEnter,this);e.detachBrowserEvent("dragleave",this._onDragLeave,this);};
...
a.prototype._attachDragEvents=function(e){e.attachBrowserEvent("dragstart",this._onDragStart,this);e.attachBrowserEvent("drag",this._onDrag,this);e.attachBrowserEvent("dragend",this._onDragEnd,this);e.attachBrowserEvent("touchstart",this._onTouchStart,this);};
a.prototype._detachDragEvents=function(e){e.detachBrowserEvent("dragstart",this._onDragStart,this);e.detachBrowserEvent("drag",this._onDrag,this);e.detachBrowserEvent("dragend",this._onDragEnd,this);e.detachBrowserEvent("touchstart",this._onTouchStart,this);};
a.prototype.onMovableChange=function(e){};
a.prototype.onDragStart=function(e){};
a.prototype.onDragEnd=function(e){};
a.prototype.onDrag=function(e){};
a.prototype.onDragEnter=function(e){};
a.prototype.onDragLeave=function(e){};
a.prototype.onDragOver=function(e){};
a.prototype.onAggregationDragEnter=function(A){};
a.prototype.onAggregationDragOver=function(A){};
a.prototype.onAggregationDragLeave=function(A){};
a.prototype.onAggregationDrop=function(A){};
a.prototype._checkMovable=function(e){var f=e.srcControl;if(f.isMovable()){D.setDraggable(f.$(),true);}};
a.prototype._onMovableChange=function(e){var f=e.getSource();if(f.isMovable()){this._attachDragEvents(f);}else{this._detachDragEvents(f);}this.onMovableChange(f);};
a.prototype._onDragStart=function(e){var f=sap.ui.getCore().byId(e.currentTarget.id);e.stopPropagation();if(sap.ui.Device.browser.firefox&&e&&e.originalEvent&&e.originalEvent.dataTransfer&&e.originalEvent.dataTransfer.setData){e.originalEvent.dataTransfer.setData('text/plain','');}this.showGhost(f,e);this.onDragStart(f);};
a.prototype._attachTouchDragEvents=function(e){e.attachBrowserEvent("touchmove",this._onTouchMove,this);e.attachBrowserEvent("touchend",this._onTouchEnd,this);};
a.prototype._detachTouchDragEvents=function(e){e.detachBrowserEvent("touchmove",this._onTouchMove,this);e.detachBrowserEvent("touchend",this._onTouchEnd,this);};
a.prototype._onTouchStart=function(e){var t=e.touches[0].pageX;var f=e.touches[0].pageY;var T,g;var h=sap.ui.getCore().byId(e.currentTarget.id);var i=function(){h.detachBrowserEvent("touchmove",T,this);h.detachBrowserEvent("touchend",g,this);};var G=function(j,k){var l=t-j;var m=f-k;return Math.sqrt((l*l)+(m*m));};T=function(e){var j=e.touches[0].pageX;var k=e.touches[0].pageY;var m=G(j,k);if(m>I){this.onDragStart(h);i.call(this);this._attachTouchDragEvents(h);}};g=function(e){i.call(this);p=false;};p=true;e.stopPropagation();h.attachBrowserEvent("touchmove",T,this);h.attachBrowserEvent("contextmenu",g,this);h.attachBrowserEvent("touchend",g,this);};
a.prototype._getTargetOverlay=function(e){if(e&&E.isInstanceOf(e,"sap.ui.dt.Overlay")){var t;if(E.isInstanceOf(e,"sap.ui.dt.AggregationOverlay")&&e.getTargetZone()){t=e;}else if(O.isInTargetZoneAggregation(e)){t=e;}return t?t:this._getTargetOverlay(e.getParent());}};
a.prototype._findTargetOverlayFromCoordinates=function(e,f){var g=document.elementFromPoint(e,f);var h=g?sap.ui.getCore().byId(g.id):undefined;return this._getTargetOverlay(h);};
a.prototype._onTouchMove=function(e){var f=sap.ui.getCore().byId(e.currentTarget.id);this.onDrag(f);var t=e.touches||e.changedTouches;var g=t[0].pageX;var h=t[0].pageY;var T=this._findTargetOverlayFromCoordinates(g,h);if(!T){return;}if(T!==o){if(o){if(E.isInstanceOf(o,"sap.ui.dt.AggregationOverlay")){this.onAggregationDragLeave(o);}else{this.onDragLeave(o);}}o=T;if(E.isInstanceOf(T,"sap.ui.dt.AggregationOverlay")){this.onAggregationDragEnter(T);}else{this.onDragEnter(T);}}if(E.isInstanceOf(T,"sap.ui.dt.AggregationOverlay")){this.onAggregationDragOver(T);}else{this.onDragOver(T);}e.stopPropagation();};
a.prototype._getValidTargetZoneAggregationOverlay=function(e){if(E.isInstanceOf(e,"sap.ui.dt.AggregationOverlay")&&e.getTargetZone()){return e;}else{return this._getValidTargetZoneAggregationOverlay(e.getParent());}};
a.prototype._onTouchEnd=function(e){var f=sap.ui.getCore().byId(e.currentTarget.id);var A=this._getValidTargetZoneAggregationOverlay(f);if(A){this.onAggregationDrop(A);}this.onDragEnd(f);this._detachTouchDragEvents(f);o=undefined;p=false;};
a.prototype.showGhost=function(e,f){if(!sap.ui.Device.browser.internet_explorer&&!sap.ui.Device.browser.edge&&!sap.ui.Device.browser.msie&&f&&f.originalEvent&&f.originalEvent.dataTransfer&&f.originalEvent.dataTransfer.setDragImage){this._$ghost=this.createGhost(e,f);this._$ghost.appendTo("#overlay-container");setTimeout(function(){this._removeGhost();}.bind(this),0);f.originalEvent.dataTransfer.setDragImage(this._$ghost.get(0),f.originalEvent.pageX-e.$().offset().left,f.originalEvent.pageY-e.$().offset().top);}};
...
a.prototype.createGhost=function(e){var g=e.getAssociatedDomRef();var $;if(!g){g=this._getAssociatedDomCopy(e);$=g;}else{$=jQuery("<div></div>");jQuery.makeArray(g).forEach(function(n){D.cloneDOMAndStyles(n,$);});}var f=jQuery("<div></div>").addClass("sapUiDtDragGhostWrapper");return f.append($.addClass("sapUiDtDragGhost"));};
a.prototype._getAssociatedDomCopy=function(e){var $=jQuery("<div></div>");e.getAggregationOverlays().forEach(function(A){A.getChildren().forEach(function(C){var f=C.getAssociatedDomRef();if(f){D.cloneDOMAndStyles(f,$);}else{D.cloneDOMAndStyles(this._getAssociatedDomCopy(C),$);}},this);},this);return $;};
a.prototype.getGhost=function(){return this._$ghost;};
a.prototype._onDragEnd=function(e){var f=sap.ui.getCore().byId(e.currentTarget.id);this._removeGhost();this._clearAllScrollIntervals();this.onDragEnd(f);e.stopPropagation();};
a.prototype._onDrag=function(e){var f=sap.ui.getCore().byId(e.currentTarget.id);this.onDrag(f);e.stopPropagation();};
a.prototype._onDragEnter=function(e){var f=sap.ui.getCore().byId(e.currentTarget.id);if(O.isInTargetZoneAggregation(f)){if(!this.onDragEnter(f)){e.stopPropagation();}}e.preventDefault();};
a.prototype._onDragLeave=function(e){var f=sap.ui.getCore().byId(e.currentTarget.id);if(O.isInTargetZoneAggregation(f)){if(!this.onDragLeave(f)){e.stopPropagation();}}e.preventDefault();};
a.prototype._onDragOver=function(e){var f=sap.ui.getCore().byId(e.currentTarget.id);if(O.isInTargetZoneAggregation(f)){if(!this.onDragOver(f)){e.stopPropagation();}}e.preventDefault();};
...
a.prototype._onAggregationDragEnter=function(e){var A=sap.ui.getCore().byId(e.currentTarget.id);this.onAggregationDragEnter(A);e.preventDefault();e.stopPropagation();};
a.prototype._onAggregationDragOver=function(e){var A=sap.ui.getCore().byId(e.currentTarget.id);this.onAggregationDragOver(A);e.preventDefault();e.stopPropagation();};
a.prototype._onAggregationDragLeave=function(e){var A=sap.ui.getCore().byId(e.currentTarget.id);this.onAggregationDragLeave(A);e.preventDefault();e.stopPropagation();};
a.prototype._onAggregationDrop=function(e){var A=sap.ui.getCore().byId(e.currentTarget.id);this.onAggregationDrop(A);e.preventDefault();e.stopPropagation();};
var b=100;var c=20;var d=50;
a.prototype._clearScrollInterval=function(e,s){if(this._mScrollIntervals[e]){window.clearInterval(this._mScrollIntervals[e][s]);delete this._mScrollIntervals[e][s];}};
a.prototype._clearScrollIntervalFor=function(e){if(this._mScrollIntervals[e]){Object.keys(this._mScrollIntervals[e]).forEach(function(s){this._clearScrollInterval(e,s);},this);}};
...
a.prototype._checkScroll=function($,s,e){var S;var f;var i=1;if(s==="top"||s==="bottom"){S=$.height();f=$.scrollTop.bind($);}else{S=$.width();f=$.scrollLeft.bind($);}if(s==="top"||s==="left"){i=-1;}var g=Math.floor(S/4);var t=b;if(g<b){t=g;}if(e<t){this._mScrollIntervals[$.attr("id")]=this._mScrollIntervals[$.attr("id")]||{};if(!this._mScrollIntervals[$.attr("id")][s]){this._mScrollIntervals[$.attr("id")][s]=window.setInterval(function(){var h=f();f(h+i*c);},d);}}else{this._clearScrollInterval($.attr("id"),s);}};
a.prototype._dragLeave=function(e){var A=sap.ui.getCore().byId(e.currentTarget.id);this._clearScrollIntervalFor(A.$().attr("id"));};
a.prototype._dragScroll=function(e){var A=sap.ui.getCore().byId(e.currentTarget.id);var $=A.$();var i=e.clientX;var f=e.clientY;var g=$.offset();var h=$.height();var w=$.width();var t=g.top;var l=g.left;var B=t+h;var r=l+w;this._checkScroll($,"bottom",B-f);this._checkScroll($,"top",f-t);this._checkScroll($,"right",r-i);this._checkScroll($,"left",i-l);};
...
a.prototype._removeDragScrollHandler=function(e){var A;if(E.isInstanceOf(e,"sap.ui.dt.AggregationOverlay")){A=e;}else{A=e.srcControl;}var f=A.getDomRef();if(f){f.removeEventListener("dragover",this._dragScrollHandler,true);}};
return a;},true);
...
a.prototype.run=function(){return this._setup().then(function(){this._mResult=this.createSuite("Element Enablement Test");var e=this.addGroup(this._mResult.children,this.getType(),"Given that an DesignTime is created for "+this.getType());this._testAggregations(e.children);this._mResult=this.aggregate(this._mResult);return this._mResult;}.bind(this));};
...
a.prototype._setup=function(){window.clearTimeout(this._iTimeout);this._bNoRenderer=false;this._bErrorDuringRendering=false;return new Promise(function(r,R){this._oElement=this._createElement();try{this._oElement.getRenderer();}catch(e){this._bNoRenderer=true;}if(!this._bNoRenderer){try{this._oElement.placeAt(this._getTestArea().get(0));sap.ui.getCore().applyChanges();}catch(e){this._bErrorDuringRendering=true;}if(!this._bErrorDuringRendering){this._oDesignTime=new D({rootElements:[this._oElement]});this._oDesignTime.attachEventOnce("synced",function(){sap.ui.getCore().applyChanges();if(this.getTimeout()){this._iTimeout=window.setTimeout(function(){r();},this.getTimeout());}else{r();}},this);}else{r();}}else{r();}}.bind(this));};
