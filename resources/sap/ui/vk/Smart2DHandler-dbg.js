	...
	Smart2DHandler.prototype.callbackHover = function() {
		...
		if (this._highlightedNodeId !== hitTestedNodeId) {
			...
			if (this._highlightedNodeId !== sap.ve.dvl.DVLID_INVALID) {
				...
				var tintColor = this._highlightedNodeId === this._previousColoring.nodeId ? this._previousColoring.color : 0;
				this._viewport.showHotspots(this._highlightedNodeId, true, tintColor);
			}
			// If the newly hit node is a hotspot, we tint it
			if (nodeHierarchy.getHotspotNodeIds().indexOf(hitTestedNodeId) !== -1) {
				// Retrieving and saving the hotspot color before tinting it as a result of hover gesture
				var nodeProxy = nodeHierarchy.createNodeProxy(hitTestedNodeId);
				this._previousColoring = {
					nodeId: hitTestedNodeId,
					color: nodeProxy.getTintColorABGR()
				};
				nodeHierarchy.destroyNodeProxy(nodeProxy);
				this._viewport.showHotspots(hitTestedNodeId, true);
			}
			// update the reference to the currently highlighted node
			this._highlightedNodeId = hitTestedNodeId;
		}
