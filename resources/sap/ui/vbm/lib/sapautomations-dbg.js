...
VBI.FlyToHandler = function(dat) {
	...
	flyToHandler.start = function() {
		...
		var lonlat = VBI.MathLib.DegToRad([
			parseFloat(flyToHandler.m_x), parseFloat(flyToHandler.m_y)
		]);
		var scene = flyToHandler.m_Ctx.m_SceneManager.GetSceneByName(flyToHandler.m_scene);
		if (scene) {
			var lod = flyToHandler.m_lod;

			flyToHandler.m_roundDown = true; //avoid fractional LOD to avoid zoom out animation at the end

			if (flyToHandler.m_roundDown) {
				lod = Math.floor(lod);
			}
			...
			if (lod != Math.floor(lod)) {
				setTimeout(function() {
					scene.AnimateZoomToGeo(lonlat, Math.floor(lod), 5);
				}, 20);
			}
		}
		return true;
