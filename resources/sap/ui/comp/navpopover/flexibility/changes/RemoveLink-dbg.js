...
sap.ui.define([
	...
	RemoveLink.applyChange = function(oChange, oNavigationPopover, mPropertyBag) {
		...
		oNavigationPopover.getAvailableActions().some(function(oAvailableAction) {
			if (oAvailableAction.getKey() === oChangeContent.key) {
				...
				oNavigationPopover._updateAvailableAction(oAvailableAction, oChange.getLayer());
				return true;
			}
		});
		return true;
	...
	RemoveLink.discardChangesOfLayer = function(sLayer, oNavigationPopover) {
		oNavigationPopover._discardAvailableActions(sLayer);
	};
