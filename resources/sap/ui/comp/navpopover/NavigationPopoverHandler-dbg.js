...
sap.ui.define([
	'jquery.sap.global', 'sap/ui/comp/library', "sap/ui/base/ManagedObject", './SemanticObjectController', 'sap/ui/model/json/JSONModel', 'sap/ui/core/Control', './Factory', './NavigationPopover', './Util', 'sap/m/VBox', './LinkData', 'sap/m/MessageBox', 'sap/ui/comp/personalization/Controller', 'sap/ui/comp/personalization/Util', './FlexHandler'
], function(jQuery, CompLibrary, ManagedObject, SemanticObjectController, JSONModel, Control, Factory, NavigationPopover, Util, VBox, LinkData, MessageBox, Controller, PersonalizationUtil, FlexHandler) {
	"use strict";
	...
	NavigationPopoverHandler.prototype._createPopover = function() {
		...
		this._oPopover._getFlexHandler().setInitialSnapshot(FlexHandler.convertArrayToSnapshot("key", oModel.getProperty("/availableActions")));
		...
		// Update the "Link Personalization" Text after "addDependent" is called and all changes are applied to NavigationPopover
	...
	NavigationPopoverHandler.prototype._onAvailableActionsPersonalizationPress = function(oEvent) {
		...
		this._oPopover._getFlexHandler().openSelectionDialog(oNavigationPopover).then(function() {
	...
	NavigationPopoverHandler.prototype._updateVisibilityOfAvailableActions = function(aMAvailableActions) {
		...
		// Update the 'visible' attribute only for storable availableActions
		...
		aMValidAvailableActions.forEach(function(oMAvailableAction) {
			// In case of many links do not show them as 'Related Apps', except the links without 'key' which should be shown always.
	 ...
	 * @private
	 */
	NavigationPopoverHandler.prototype._updateAvailableActionsPersonalizationText = function() {
		var oModel = this.getModel("$sapuicompNavigationPopoverHandler");
		var aMValidAvailableActions = Util.getStorableAvailableActions(oModel.getProperty("/availableActions"));

		// Do not show any text if there are no available actions
		if (aMValidAvailableActions.length === 0) {
			oModel.setProperty("/navigationTarget/availableActionsPersonalizationText", undefined);
			return;
		}
		...
		if (!bEnableAvailableActionsPersonalization) {
			oModel.setProperty("/navigationTarget/availableActionsPersonalizationText", undefined);
			return;
		}

		// Default text is "Define Links"
		...
		// If changes exist then set "More" text
		if (this._oPopover) {
			var oSnapshotOfChanges = this._oPopover._getFlexHandler().determineSnapshotOfChangedAvailableActions();
			if (!jQuery.isEmptyObject(oSnapshotOfChanges)) {
				sAvailableActionsPersonalizationText = sap.ui.getCore().getLibraryResourceBundle("sap.ui.comp").getText("POPOVER_DEFINE_MORE_LINKS");
			}
		}

		oModel.setProperty("/navigationTarget/availableActionsPersonalizationText", sAvailableActionsPersonalizationText);
	};
