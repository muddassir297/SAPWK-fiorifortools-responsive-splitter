...
sap.ui.define([
			...
			if (sap.ui.table.Table && this._oTable instanceof sap.ui.table.Table) {
				...
				if (this.oRows.aKeys) {
					...
					for (j = 0; j < aItems.length; j++) {
						var sKey = encodeURI(aItems[j]);	//key of the item must be encoded before we search the item in the table row keys.
						...
						// sKey = this._oTable.getModel().createKey("VL_SH_FAC_OPP_SH_PPV", { OPVAR: " 123", OPVTX: "foo" });
						...
						for (i = 0; i < this.oRows.aKeys.length; i++) {
							...
							if (bIsRow || // either the rowKey is equal the token key or we search if the main key with the value is part of the
											// rowKey
	...
	ValueHelpDialog.prototype._createTokenizer = function() {
		...
		this._oSelectedTokens = new sap.m.Tokenizer({
			tokenUpdate: jQuery.proxy(function(oControlEvent) {
...
// }, this));
	...
	ValueHelpDialog.prototype._addRemoveTokenByKey = function(sKey, oRow, bAdd) {
		sKey = decodeURI(sKey);	// key of the added orremoved item must be decoded
		if (bAdd) {
