...
sap.ui.define( [
	...
	DateRangeType.getIntField = function( oInstance ) {
		return new Input( Type._createStableId( oInstance, "field" ), {
			...
			value: {
				path: "$smartEntityFilter>value1",
				type: new sap.ui.model.type.NullableInteger({}, { minimum: 0 })
			},
			textAlign: "End",
			//type: "Number",
			width: "100%"
	...
	DateRangeType.prototype._updateProvider = function( oJson, bSync ) {
		...
		if ( this.oModel.getData().currentoperation.languageText ) {
			...
			this.oModel.setProperty( "/currentoperation/formattedText", sFormattedText );
	...
	DateRangeType.prototype.initializeFilterItem = function() {
		this._oInput = new sap.m.Input( Type._createStableId( this ), {
			value: "{$smartEntityFilter>currentoperation/formattedText}",
			//tooltip: "{$smartEntityFilter>currentoperation/formattedText}",
			showValueHelp: true,
			showSuggestion: true,
			maxSuggestionWidth: "auto",
			valueHelpRequest: jQuery.proxy( this._toggleOpen, this )
		} );
		...
		this._oInput.attachSuggestionItemSelected( jQuery.proxy( function( oEvent ) {
		...
		this._oInput.attachChange( jQuery.proxy( function( oEvent ) {
		...
		this._oInput.attachBrowserEvent( "focusin", jQuery.proxy( function( oEvent ) {
	...
	DateRangeType.prototype._toggleOpen = function( oEvent ) {
		...
		if ( !this._oPopup ) {
			...
			this._oPopup = new sap.m.ResponsivePopover( {
				...
				//					text: "Cancel", 
				//					press: jQuery.proxy(function(oEvent){
				//						//TODO restore the  old currentoperation/condition
				//						if (this._oOrgCondition) {
				//							this.setCondition(this._oOrgCondition);
				//						}
				//						this._oPopup.close();
				//					}, this)}),
				afterClose: jQuery.proxy( function( oEvent ) {
					if ( this._oOrgCondition ) {
						this._oOrgCondition = null;
						delete this._oOrgCondition;
					}
				}, this ),
		...
		this._oOrgCondition = this.getCondition();
		if ( !this._oPopup.isOpen() ) {
			this._oPopup.openBy( this._oInput._oValueHelpIcon );
		} else {
			this._oPopup.close();
		}
	};
