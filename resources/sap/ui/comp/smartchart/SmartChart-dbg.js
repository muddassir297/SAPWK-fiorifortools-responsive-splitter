...
sap.ui.define([
	...
	var SmartChart = VBox.extend("sap.ui.comp.smartchart.SmartChart", /** @lends sap.ui.comp.smartchart.SmartChart.prototype */
	{
		metadata: {
			...
			properties: {
				...
				/**
				 * Factory function for custom actions on item level within details popover. Property must be of type <code>function</code> which
				 * returns an instance of <code>sap.m.Link</code>.
				 * 
				 * @experimental since 1.46
				 */
				detailsItemActionFactory: {
					type: "any", // Will be of type function in the future, once supported for properties
					group: "Misc",
					defaultValue: null
				},
				/**
				 * Factory function for custom actions on list level within details popover. Property must be of type <code>function</code> which
				 * returns an instance of <code>sap.m.Link</code>.
				 * 
				 * @experimental since 1.46
				 */
			...
			events: {
				 ...
				 *        is prevented
				 * @param {Array} oControlEvent.getParameters.bindingParams.filters The combined filter array containing a set of sap.ui.model.Filter
				 *        instances of the SmartChart and SmartFilter controls; can be modified by users to influence filtering
				 * @param {Array} oControlEvent.getParameters.bindingParams.sorter An array containing a set of sap.ui.model.Sorter instances of the
				 *        SmartChart control (personalization); can be modified by users to influence sorting
				 * @public
				 */
				beforeRebindChart: {
	...
	SmartChart.prototype._createTooltip = function() {
		if (this._oChart) {
			this._oChart.setVizProperties({
				"interaction": {
					"behaviorType": this._getBehaviorTypeForDataSelection()
				},
				"tooltip": {
					"visible": true
				}
	...
	SmartChart.prototype.setShowDetailsButton = function(bFlag) {
		...
		if (this._oDetailsButton) {
			...
			// Change behaviorType to get tooltiptext information during datapoint selection
			// This is just needed when details popover is enabled in order to show correct format
			this._oChart.setVizProperties({
				"interaction": {
					"behaviorType": this._getBehaviorTypeForDataSelection()
				}
	...
	SmartChart.prototype._getBehaviorTypeForDataSelection = function() {
		if (this.getShowDetailsButton()) {
			return "noHoverBehavior";
		} else {
			return null;
		}
	};
	...
	SmartChart.prototype.getDrillStackFilters = function() {
		...
		aDrillStack.forEach(function(oStackEntry, index, aDrillStack) {
	...
	SmartChart.prototype._addDetailsButton = function() {
		...
		if (!this._oDetailsButton /* && this.getShowDetailsButton() */) {
			...
			// Remember: Columns get deselected after drill up or down happened!
			this._oChart.attachDrilledUp(function(oEvent) {
	...
	SmartChart.prototype._updateDetailsText = function(oEvent) {
		...
		var bHasNoData = oVizFrame ? oVizFrame._isNodata : false;
		var sNoDataText = this.getNoData();

		if (bHasNoData && sNoDataText.length > 0) {
			// update the no data text
			oVizFrame._updateDescription({
				title: sNoDataText
			});
		}

		var sDetailsText, iSelectedCategories = this._oChart.getSelectedDataPoints().count;
		if (iSelectedCategories > 0) {

			var sLabelText = this._oRb.getText("CHART_DETAILSBTN_LABEL");
			var sBracketsText = this._oRb.getText("CHART_DETAILSBTN_LABEL_BRACKETS", [
				iSelectedCategories
			]);

			if (sap.ui.getCore().getConfiguration().getRTL()) {
				// RTL mode
				// set back to LTR due to Dev Guide and manually change order
				this._oDetailsButton.setTextDirection(sap.ui.core.TextDirection.LTR);
				sDetailsText = sBracketsText + " " + sLabelText;
			} else {
				// LTR mode
				sDetailsText = sLabelText + " " + sBracketsText;
			}

		} else {
			sDetailsText = this._oRb.getText("CHART_DRILLDOWNDETAILSPAGE_TITLE");
		}
		this._oDetailsButton.setText(sDetailsText);

		if (this._oDetailsMasterPage && this._oDetailsMasterPage.getShowHeader()) {
			this._oDetailsMasterPage.setTitle(sDetailsText);
		}
	};
	...
	SmartChart.prototype._updateDetailsModel = function(oEvent) {
		...
		var aSelectionParams = oEvent.mParameters.data;

		switch (oEvent.sId) {
			case "selectData":
				this._addDetailsModelEntry(aSelectionParams);
				break;
			case "deselectData":
				if (aSelectionParams) {
					this._removeDetailsModelEntry(aSelectionParams);
				} else {
					// handle fireDeselect during drill-up / drill-down
					// clean details model
					this._aDetailsEntries = [];
				}
				break;
	...
	SmartChart.prototype._removeDetailsModelEntry = function(aDeselectionParams) {

		var aDeselectTooltips = [];
		var aEntryTooltips = [];

		// Get tooltips from deselection
		aDeselectionParams.forEach(function(oParam) {
			aDeselectTooltips.push(oParam.tooltipText);
		});

		// Get tooltips from current model entries
		this._aDetailsEntries.forEach(function(oParam) {
			aEntryTooltips.push(oParam.tooltipText);
		});

		// For each aDeselectTooltip run through aEntryTooltips
		// and compare. If same, then delete.
		aDeselectTooltips.forEach(function(aTooltip) {

			// For each details entry compare objects to aTooltip entry
			aEntryTooltips.forEach(function(aDetail, entryIndex) {

				function compareObjects(oDetail, index, array) {
					return JSON.stringify(oDetail) === JSON.stringify(aTooltip[index]);
				}

				var isEqual = aDetail.every(compareObjects);

				if (isEqual) {
					this._aDetailsEntries.splice(entryIndex, 1);
				}
	...
	SmartChart.prototype._addDetailsModelEntry = function(aSelectionParams) {

		aSelectionParams.forEach(function(oParam) {
			this._aDetailsEntries.push(oParam/* .tooltipText */);
		}.bind(this));
	};
	...
	SmartChart.prototype._bindDetailsListAggregation = function() {
		...
		/*
		 * this._oDetailsMasterList.bindAggregation("items", "/items", function(sId, oContext) { var measure; var tecMeasureName; var
		 * displayMeasureName; var measureUnit; var dimNames = ""; // Find the specific measure and its value for this oContext and use it in template
		 * aMeasures.some(function(m) { // Check value in measures array of oContet object and if we have a match, copy the value for the list item.
		 * if (oContext.getObject().measures[0] == m) { measure = oContext.getProperty(m); tecMeasureName = m; displayMeasureName =
		 * this._oChart.getMeasureByName(tecMeasureName).getUnitBinding(); // Get the measureUnit for the measure if (displayMeasureName) {
		 * measureUnit = oContext.getProperty(displayMeasureName); // Put the measure unit behind the value if (measureUnit) { measure =
		 * measure.concat(" ", measureUnit); } } return true; } }.bind(this)); // Get all dimensions except for the last one as we show this one in
		 * the title of the item for (var i = 0; i < aDimensions.length - 1; i++) { dimNames += oContext.getProperty(aDimensions[i]); if (i <
		 * aDimensions.length - 2) { dimNames += " / "; } } // Build an item template with all needed values var oDetailsItemTemplate = new
		 * sap.m.ObjectListItem({ title: oContext.getProperty(aDimensions[aDimensions.length - 1]),// last and hence deepest currently visible
		 * dimension // type: "Navigation", type: (function() { // Check if we have semantic objects for this this data point context if
		 * (this._determineSemanticObjects(oContext.getObject(), oContext).length > 0) { return sap.m.ListType.Navigation; } else { return
		 * sap.m.ListType.Inactive; } }.bind(this)()), number: measure, numberUnit: tecMeasureName, attributes: [ new sap.m.ObjectAttribute({ text:
		 * dimNames }) ] }); // Attach the semantic object navigation to each item oDetailsItemTemplate.attachPress(function(oEvent) {
		 * this._navigateToSemanticObjectDetails(oEvent); }.bind(this)); return oDetailsItemTemplate; }.bind(this));
		 */

		// NEW IMPLEMENTATION FOR DETAILSPOPOVER WITH ACCESS TO TOOLTIPTEXTS
		this._oDetailsMasterList.bindAggregation("items", "/items", function(sId, oContext) {
			// Put it at beginning for files require js loading
			...
			var oDetailsFormLayout = new sap.ui.layout.form.ResponsiveGridLayout({
				labelSpanXL: 3,
				labelSpanL: 3,
				labelSpanM: 3,
				labelSpanS: 5,// 12,
			...
			var oDetailsFormContainer = new sap.ui.layout.form.FormContainer();
			var oDetailsItemActionContainer = new sap.m.HBox({
				justifyContent: "End"
			});

			oContext.getObject().tooltipText.forEach(function(oDetail) {
				// Skip this private info
				if (oDetail.id === "_context_row_number") {
					return;
				}

				oDetailsFormContainer.addFormElement(new sap.ui.layout.form.FormElement({
					label: new sap.m.Label({
						text: oDetail.name,
						tooltip: oDetail.name
					}),
					fields: new sap.m.Label({
						text: oDetail.value,
						tooltip: oDetail.value
					})
				}));
			});

			// Create form and put things together
			var oDetailsTextForm = new sap.ui.layout.form.Form({
				formContainers: [
					oDetailsFormContainer
				],
				layout: oDetailsFormLayout
			});

			// Create Template which uses the form
			var oDetailsItemTemplate = new sap.m.CustomListItem({
				type: (function() {
					// Check if we have semantic objects for this this data point context
					if (this._determineSemanticObjectsforDetailsPopover(oContext.getObject(), oContext).length > 0) {
			...
			// FORM
			oDetailsItemTemplate.addContent(oDetailsTextForm);

			// DETAILS ITEM ACTIONS
			if (this.getDetailsItemActionFactory()) {

				var oDetailsItemAction = this.getDetailsItemActionFactory()(oContext);// Hand in the complete model context

				if (oDetailsItemAction.getMetadata().getElementName() === "sap.m.Link") {
					// proceed
					oDetailsItemActionContainer.addItem(oDetailsItemAction);
					oDetailsItemTemplate.addContent(oDetailsItemActionContainer);
				} else {
					// throw error log as we won't support other controls right now
					jQuery.sap.log.error("fnDetailsItemActionFactory returns invalid type. Only type sap.m.Link is supported as return value");
				}
			}
		...
		// Clear List Action container before opening popover
		if (this._oDetailsListActionContainer && this._oDetailsListActionContainer.getItems().length > 0) {
			this._oDetailsListActionContainer.removeAllItems();
		}
		if (this.getDetailsListActionFactory()) {
			//Get all binding contexts from current datapoint selections
			var aBindingContexts = [];

			this._oDetailsMasterList.getItems().forEach(function(oItem, index, array) {
				aBindingContexts.push(oItem.getBindingContext());
			});

			var oDetailsListAction = this.getDetailsListActionFactory()(aBindingContexts);// hand in all data from the details model

			if (oDetailsListAction.getMetadata().getElementName() === "sap.m.Link") {
				// proceed
				this._oDetailsListActionContainer.addItem(oDetailsListAction);
				// this._oDetailsMasterListScrollContainer.addContent(oDetailsListActionContainer);
			} else {
				// throw error log as we won't support other controls right now
				jQuery.sap.log.error("fnDetailsListActionFactory returns invalid type. Only type sap.m.Link is supported as return value");
			}
		}
	};
	/**
	 * sets the factory functions for details actions on item level
	 * 
	 * @param {function} fnDetailsItemActionFactory factory function for item level that must return an instance of type sap.m.Link
	 * @experimental
	 */
	SmartChart.prototype.setDetailsItemActionFactory = function(fnDetailsItemActionFactory) {

		this._validateDetailsFactory(fnDetailsItemActionFactory) ? this.setProperty("detailsItemActionFactory", fnDetailsItemActionFactory) : jQuery.sap.log.error("fnDetailsItemActionFactory must be of type function");

	};
	/**
	 * sets the factory functions for details actions on details list level
	 * 
	 * @param {function} fnDetailsListActionFactory factory function for list level that must return an instance of type sap.m.Link
	 * @experimental
	 */
	SmartChart.prototype.setDetailsListActionFactory = function(fnDetailsListActionFactory) {

		this._validateDetailsFactory(fnDetailsListActionFactory) ? this.setProperty("detailsListActionFactory", fnDetailsListActionFactory) : jQuery.sap.log.error("fnDetailsListActionFactory must be of type function");
	};
	/**
	 * validates if the passed factory is of type <code>function</code>.
	 * 
	 * @param {function} fnDetailsFactory factory function for details popover list or list item level.
	 * @private
	 */
	SmartChart.prototype._validateDetailsFactory = function(fnDetailsFactory) {

		return typeof fnDetailsFactory === "function" ? true : false;
	};
 ...
 */
	...
	SmartChart.prototype._createDetailsPopoverStructure = function() {
		...
		// Add List action structure to popover
		if (this.getDetailsListActionFactory()) {

			this._oDetailsListActionContainer = new sap.m.HBox({
				justifyContent: "End"
			});
			this._oDetailsMasterListScrollContainer.addContent(this._oDetailsListActionContainer);
		}
...
// }
	...
	SmartChart.prototype._reBindChart = function() {
		...
		var aChartInResultDimensions = arrayUnique(this._oChart.getInResultDimensions().concat(aInResultDim));

		// Keep the initial inResultDimensions
		if (!this._aInitialInResultDimensions) {
			this._aInitialInResultDimensions = aChartInResultDimensions;
		}

		// Check if some inResult params have changed during runtime
		var inResDimsAreEqual = function() {

			if (!aChartInResultDimensions) {
				return false;
			}

			if (aChartInResultDimensions.length != this._aInitialInResultDimensions.length) {
				return false;
			}

			aChartInResultDimensions.forEach(function(dim, index, array) {
				if (dim !== this._aInitialInResultDimensions[index]) {
					return false;
				}
			}.bind(this));
		...
		// Only set this value on SmartChart init or when values have changed
		// Setting this value again will create a new drill-stack, hence current drill-history gets lost!
		// For more details about this limitation see sap.chart.Chart.prototype.setInResultDimensions.
		if (!inResDimsAreEqual()) {
			this._oChart.setInResultDimensions(aChartInResultDimensions);
		}
