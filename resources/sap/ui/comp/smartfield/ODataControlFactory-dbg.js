...
sap.ui.define([
	...
	ODataControlFactory.prototype._init = function(oMetaData) {
		...
		if (this._oModel) {
			...
			var oMetadataProperty = this._getMetadataProperty();
...
// justifyContent: FlexJustifyContent.End,
	...
	ODataControlFactory.prototype._getMetadataProperty = function() {
	...
	ODataControlFactory.prototype._checkLink = function() {
		var oInfo = this._oParent.getBindingInfo("url"),
			oProperty = this._getMetadataProperty();
	...
	ODataControlFactory.prototype._createLink = function() {
		...
		if (oBindingInfo) {
			...
			if (oMetaData.annotations.text && (oProperty.type === "Edm.String")) {
				mAttributes.text = {
					parts: [
						oMetaData.path, this._oHelper.getEdmDisplayPath(oMetaData)
					],
					formatter: this._formatText
				};
			} else if (ODataControlFactory.isSpecialLink(oProperty)) {
				var fnFormatter = ODataControlFactory[ODataControlFactory._getFormatterFunctionName(oProperty)];
	...
	ODataControlFactory._getFormatterFunctionName = function(oProperty) {
		return "_format" + MetadataAnalyser.getDisplayFormat(oProperty);
	};
