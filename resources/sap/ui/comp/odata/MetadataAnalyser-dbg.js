...
sap.ui.define([
	...
	MetadataAnalyser.getDisplayFormat = function(oProperty) {
		...
		var oDisplayFormat = oProperty["com.sap.vocabularies.Common.v1.IsUpperCase"];

		if (MetadataAnalyser.isAnnotationTrue(oDisplayFormat)) {
			return "UpperCase";
		}

		if (MetadataAnalyser.isDigitSequence(oProperty)) {
			return "NonNegative";
		}
	...
	MetadataAnalyser.isAnnotationTrue = function(oAnnotation) {
		return !!oAnnotation && (oAnnotation.Bool === "true");
	};
	...
	MetadataAnalyser.isStringTypePropertyAnnotated = function(oProperty, sTerm) {
		return MetadataAnalyser.isPropertyStringType(oProperty) && MetadataAnalyser.isAnnotationTrue(oProperty[sTerm]);
	};

	/**
	 * Checks whether <code>oProperty</code> is annotated as a digit sequence. Intended for
	 * <code>Edm.String</code> fields that are internally stored as <code>NUMC</code> (numeric text) data type.
	 *
	...
	MetadataAnalyser.isDigitSequence = function(oProperty) {
		var sTerm = "com.sap.vocabularies.Common.v1.IsDigitSequence";
		return MetadataAnalyser.isStringTypePropertyAnnotated(oProperty, sTerm);
	};
	...
	MetadataAnalyser.isEmailAddress = function(oProperty) {
		var sTerm = "com.sap.vocabularies.Communication.v1.IsEmailAddress";
		return MetadataAnalyser.isStringTypePropertyAnnotated(oProperty, sTerm);
	};
	...
	MetadataAnalyser.isPhoneNumber = function(oProperty) {
		var sTerm = "com.sap.vocabularies.Communication.v1.IsPhoneNumber";
		return MetadataAnalyser.isStringTypePropertyAnnotated(oProperty, sTerm);
	};
	 ...
	 * @param {object} oProperty The OData property from the meta model
	 * @returns {boolean} <code>true</code>, if the OData property is annotated with the following
	 * <code>"Core.IsUrl": { "Bool" : "true" }</code> annotation.
	 * @protected
	 * @since 1.46
	 */
	MetadataAnalyser.isUrl = function(oProperty) {
		var sTerm = "Core.IsUrl";
		return MetadataAnalyser.isStringTypePropertyAnnotated(oProperty, sTerm);
	};
	...
	MetadataAnalyser.isValueListWithFixedValues = function(oProperty) {
		...
		return MetadataAnalyser.isAnnotationTrue(oProperty[sTerm]);
	};
	...
	MetadataAnalyser.prototype._parseProperty = function(oProperty, oEntityDef, sParentPropertyName) {
		...
		// hidden in filter
		oField.hiddenFilter = this._isHiddenFilter(oProperty);
	 ...
	 * @public
	 */
	MetadataAnalyser.prototype._getAllFilterableFieldsByEntity = function(sEntity, bIsEntitySet) {
	...
	MetadataAnalyser.prototype.getAllFilterableFieldsByEntitySetName = function(sEntitySetName) {
		...
		return this._getAllFilterableFieldsByEntity(sEntitySetName, true);
	};
				...
				if (aAnnotationData) {
					...
					if (aAnnotationData.Visualizations) {
						...
						for (i = 0; i < iLen; i++) {
							...
							if (!oLineItemAnnotation && sItemPath.indexOf("com.sap.vocabularies.UI.v1.LineItem") > -1) {
							...
							} else if (!oChartAnnotation && sItemPath.indexOf("com.sap.vocabularies.UI.v1.Chart") > -1) {
	...
	MetadataAnalyser.getSemanticObjectsFromProperty = function(oProperty) {
		...
		for ( var sAttr in oProperty) {
			if (jQuery.sap.startsWith(sAttr, "com.sap.vocabularies.Common.v1.SemanticObject")) {
				if (jQuery.sap.endsWith(sAttr, "com.sap.vocabularies.Common.v1.SemanticObject")) {
