...
sap.ui.define([
	...
	SplitField.applyChange = function(oChange, oControl, mPropertyBag) {
		...
		var oParent = oModifier.bySelector(oChangeDefinition.content.parentSelector, oAppComponent);
		var aNewElementIds = oChangeDefinition.content.newElementIds.slice();
		var aFields = oModifier.getAggregation(oSourceControl, "fields");
		var iLabelElementIndex = oModifier.getProperty(oSourceControl, "elementForLabel");
		var iControlIndex = oParent.indexOfAggregation("formElements", oSourceControl);
		...
		for (var i = 0, n = aFields.length; i < n; i++) {
			if (i !== iLabelElementIndex) {
				...
				oModifier.insertAggregation(oNewGroupElement, "elements", aFields[i], 0, oView, true);
				oModifier.insertAggregation(oParent, "groupElements", oNewGroupElement, iControlIndex + i, oView, true);
				...
				oModifier.removeAggregation(oSourceControl, "elements", aFields[i]);
				oModifier.insertAggregation(oSourceControl, "elements", aFields[i], 0, oView, true);
