...
sap.ui.define([
...
], function(jQuery, ComboBox, DatePicker, DateRangeSelection, TimePicker, Input, MultiComboBox, MultiInput, SearchField, Token, MetadataAnalyser, ValueHelpProvider, ValueListProvider, Filter, JSONModel, ODataType, FormatUtil, EventProvider, IdentifierUtil) {
	"use strict";
	...
	FilterProvider.prototype._intialiseMetadata = function() {
		...
		aODataFilterGroups = this._oMetadataAnalyser.getAllFilterableFieldsByEntitySetName(this.sEntitySet);
	...
	FilterProvider.prototype._createInitialModelForField = function(oJSONData, oFilterFieldMetadata, bUseDefaultValues) {
		...
		if (oFilterFieldMetadata.filterRestriction === sap.ui.comp.smartfilterbar.FilterType.single) {
			...
			if (bHasDefaultFilterValue) {
				...
				sLowValue = bIsDateTimeType ? new Date(oDefaultFilterValue.low) : oDefaultFilterValue.low;
			}
			...
			if (bHasDefaultFilterValue) {
				oDefaultFilterValue = aDefaultFilterValues[0];
				sLowValue = bIsDateTimeType ? new Date(oDefaultFilterValue.low) : oDefaultFilterValue.low;
				sHighValue = bIsDateTimeType ? new Date(oDefaultFilterValue.high) : oDefaultFilterValue.high;
			}
				...
				while (iLength--) {
					...
					if (bIsRangeField) {
						oItem = {
							...
							"value1": bIsDateTimeType ? new Date(oDefaultFilterValue.low) : oDefaultFilterValue.low,
						...
						oItem = {
							key: bIsDateTimeType ? new Date(oDefaultFilterValue.low) : oDefaultFilterValue.low,
...
// });
	...
	FilterProvider.prototype._createFilterControlId = function(oFieldViewMetadata) {
		...
		var sGroupName = IdentifierUtil.replace(oFieldViewMetadata.groupName || "");
		var sName = IdentifierUtil.replace(oFieldViewMetadata.name);

		return sFilterBarName + "-filterItemControl" + sGroupName + "-" + sName;
	};
