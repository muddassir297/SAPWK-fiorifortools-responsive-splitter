...
sap.ui.define([], function() {
	...
	return {
		annotations: {
			/**
			 * NonFilterableProperties defines whether a <code>Property</code> can be used for filtering data. All properties are filterable by default. In order to disable
			 ...
			 * </pre>
			 *
			 * RequiredProperties defines the filter field as mandatory filter. Filter fields having this annotation must be a part of the <code>$filter</code> request
			...
			/**
			 * Enum member MultiValue defines whether multiple values can be used in a single filter. With multi-value filtering, more that one "equals" condition can be
			 ...
			 * </pre>
			 *
			 * Enum member SingleValue restricts the filter to only <b>one</b> single value entry. With single-value filtering, you cannot define more than one "equals"
			 ...
			 * </pre>
			 *
			 * Enum member SingleInterval restricts the filter to specify an interval, for example, a date interval for filtering the data present between the given dates.
