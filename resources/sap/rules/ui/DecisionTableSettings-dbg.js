...
sap.ui.define([
	...
	function(jQuery, library, Control, SimpleForm, Label, Switch, Select, MessageBox, Table, Text, CheckBox, Input, Button,
			...
			metadata: {
				...
				properties: {
					cellFormat: {
						...
						defaultValue: sap.rules.ui.DecisionTableCellFormat.Text
					},
					hitPolicies: {
		...
		sap.rules.ui.DecisionTableSettings.prototype._isTextMode = function () {
			var sCellFormat = this.getCellFormat();
			if (sCellFormat === sap.rules.ui.DecisionTableCellFormat.Both || sCellFormat === sap.rules.ui.DecisionTableCellFormat.Guided) {
				return "30%";
			}
			return "0%";
		};
			...
			var oInput = new Input({
				...
				valueHelpRequest: function(oEvent) {
					...
					var _handleValueHelpOpen = function(){
						...
						oSelectDialog.setModel(oInput.getModel("settingsModel"), "settingsModel");
						oSelectDialog.open();
					}.bind(this);
		...
		sap.rules.ui.DecisionTableSettings.prototype._applySettingsModelChangesToOData = function(oDialog) {
			...
			var sRuleId = this.getBindingContext().getProperty("Id");
			var sVersion = this.getBindingContext().getProperty("Version");
				...
				for (var i = 0; i < aSettingsModelColumns.length; i++) {
					...
					if (settingsModelColumn.Status) {
						...
						if (settingsModelColumn.Condition) {
							...
							newExpression = settingsModelColumn.Condition.parserResults.converted ?
						...
						if (settingsModelColumn.Status === "U") {
							...
							if (settingsModelColumn.Condition) {
								...
								var newFixedOperator = settingsModelColumn.Condition.FixedOperator;
								var newValueOnly = settingsModelColumn.Condition.ValueOnly;
								var updatedConditionOdata = {
									Id: Id,
									Expression: newExpression,
									FixedOperator: newFixedOperator,
