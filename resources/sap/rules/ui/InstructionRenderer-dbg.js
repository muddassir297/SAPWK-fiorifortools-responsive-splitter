sap.ui.define([
	...
	return Control.extend("sap.rules.ui.InstructionRenderer", {
		metadata : {
			...
			aggregations: {
				_content: {type : "sap.ui.core.Control", multiple: true, visibility : "hidden"}
			},
			events: {
				"change": {}
			},
		...
		getExpression: function() {
			...
			if (!instructions[instructions.length - 1].token) {
		...
		_createSelectionControl: function(instruction, nextCreateFunction, localPath, fullPath) {
			...
			return {controls:[ctrl], needLineBreak:false};
		},
		...
		_createAddRepetitiveControl : function(instruction, nextCreateFunction, localPath, fullPath) {
			...
			return {controls:ctrlCollection, needLineBreak:false};
		},
			...
			var ctrl = new Input({
				value: {
					...
					type: new sap.ui.model.type.String(
						null, 
						{search: this.VALIDATION_REGEX}) // prevent illegal characters
				},
				width: "auto",
		...
		_createNonEditableLogicalOperator: function(instruction, nextCreateFunction, localPath, fullPath) {
			...
			if (!this._getIndent()) { controls.push(lineBreak); }
			controls.push(ctrl);
			return {controls:controls, needLineBreak:true};
		},
		...
		_createValueRange: function(instruction) {
			...
			if (instruction.valueOptions && instruction.valueOptions.type === "Range") {
				...
				if (rangeValues.from) { range.minimum = rangeValues.from; }
				if (rangeValues.to) { range.maximum = rangeValues.to; }
			}
		...
		_syncToken: function(localPath){
			var model = this._internalModel, addStartQuote = false, addEndQuote = false;
			var newValue = model.getProperty(localPath + this.TEXT).toString();
			
			if (newValue === "'")          { addStartQuote = true; }   // add only quote for this special case
			if (newValue.substr(0, "'") !== "'"/*.startsWith("'")*/) { addStartQuote = true; }
			if (newValue.substr(newValue.length-1, "'") !== "'")/*.endsWith("'"))*/   { addEndQuote = true;   }
			
			if (addStartQuote) { newValue = "'" + newValue; }
			if (addEndQuote)   { newValue = newValue + "'"; }
