...
sap.rules.ui.parser.ruleBody.lib.ruleServices.lib = (function(){
	 ...
	 * @param flags {object}
	 */
	var updateFlags = function(ruleId, flags){
	 ...
	 * @returns
	 */
	var validateRule = function(oDataRule, vocaId, vocaDataProvider, flags)
	{
		...
		updateFlags(getPropertyFromObject(oDataRule,ODATA_PAYLOAD_PROPERTY_ID), flags);
		result = ruleBodyServ.process(convertedRule.ruleBody, vocaId, vocaDataProvider, flags, outputName, null, oDataRule);
		return result;
	};
	 ...
	 * @param vocaDataProvider : Vocabulary data provider instance.
	 */
	var validateRuleService = function(oData, vocaId, vocaDataProvider){
		...
		if(!(validateRuleServiceResult[ruleBodyConstants.RULE_STATUS] === ruleBodyConstants.RULE_ERROR && Object.keys(failedRulesObj).length === 0)){
			...
			for(i = 0; i < oDataRulesArr.length; i++){
				...
				if(!failedRulesObj[ruleID]){
					validateRuleResult = validateRule(getPropertyFromObject(ruleToValidate,ODATA_PAYLOAD_PROPERTY_CONTENT), vocaId, vocaDataProvider, null);
	...
	var executeRuleValidation = function(requestBodyPayload){
	    ...
	    if(!vocabulary){
	        ...
	        responseObj = validateRule(ruleContent, vocaId, vocaDataProvider, flags);
	    }
	    return responseObj;
	};
	...
	var executeRuleServiceValidation = function(requestBodyPayload){
		...
		if(!vocabulary){
		    ...
		    vocaId = getPropertyFromObject(vocaContent,ODATA_PAYLOAD_PROPERTY_ID);
		    vocaDataProvider = initVocaDataProvider(vocaId, vocaContent);
		    responseObj = validateRuleService(requestBodyPayload, vocaId, vocaDataProvider);	    
		}
		return responseObj;
	};
