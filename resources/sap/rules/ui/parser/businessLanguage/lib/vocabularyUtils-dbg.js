...
sap.rules.ui.parser.businessLanguage.lib.vocabularyUtils.lib = (function() {
	...
	****************************************************************/
	 ...
	 * P R I V A T E
	****************************************************************/
	 vocabularyUtilsLib.prototype.getAltTypedAttributes = function (vocaRTServ, vocaName, objName, attrName, isCollection, typeArr) {
		var attrArr = vocaRTServ.getObjectAttributesNames (vocaName, objName, attrName);
		if (typeArr.length === 0) {
			return attrArr;
		}
	...
	****************************************************************/
	 ...
	 vocabularyUtilsLib.prototype.getAltTypedTerms = function(vocaRTServ, vocaName, startWith, typeArr, context, isCurrent, completeCollection, isDeprecatedAllContext, isAutoComplete, isInsideWhere, isAllContext, modelMngrTermMode) {
		    ...
		    for (i=0; i<termArr.length; i++) {
				...
				// Attributes of the context are provided as is. Hence the corresponding terms should'nt be provided
				 if( !isCurrent && !completeCollection && context !== null && termInfo.context !== null && termInfo.expression.indexOf(context) === 0 && termInfo.isConditionalContext === false &&
		   				 context !== termInfo.contex && termInfo.businessDataType !== null && (termInfo.expression.split(".").length === (context.split(".").length + 1))){
					 continue;
				 }
			   	...
			   	//If no context was provided it means we should return all terms which are not conditional context terms
			   	 ...
			   	 if ((context === null && termInfo.isConditionalContext === false && !isCurrent) || 
			   		 (context !== null && termInfo.context !== null && context.indexOf(termInfo.context) === 0 && context !== termInfo.context && !isCurrent) ||
			   		 (context !== null && termInfo.expression.indexOf(context) === 0 && termInfo.isConditionalContext === false && isCurrent)||
			   		 (context !== null && termInfo.expression.indexOf(context) === 0 && termInfo.modifiers.hasOwnProperty("current") && termInfo.modifiers.current) ||
	...
	****************************************************************/
	 ...
	 ****************************************************************/
	 vocabularyUtilsLib.prototype.isTermPrefix = function(term_prefix, vocaRTServ, vocaName, modelMngrTermMode) {
		...
		var terms = this.getAltTypedTerms(vocaRTServ, vocaName, term_prefix, [], null, null, null, null, null, null, null, termModeFlag);	
		if (terms.length > 0) {
			return true;
		}
				...
				while (pathSegments.length > 0) {
					...
					if (!assoc) {
						...
						if (attr) {
							...
							result.attribute.altAttr = this.getAltTypedAttributes (vocaRTServ, vocaName, curObject, curSegment, result.isCollection, typeArr);
	 ...
	 ****************************************************************/
	 ...
	 vocabularyUtilsLib.prototype.getNavigationCompletion = 
		 function(startWith, vocaRTServ, vocaName, typeArr, valueListAttribute, context, isCurrent, completeCollection, isDeprecatedAllContext, isInsideWhere, isAllContext, modelMngrTermMode) {
		var result = {
				altDO: [],
				altAlias: [],
				altValueList: []
		...
		result.altDO    = this.getAltTypedTerms(vocaRTServ, vocaName, startWith, typeArr, context, isCurrent, completeCollection, isDeprecatedAllContext, true, isInsideWhere, isAllContext, termModeFlag);
