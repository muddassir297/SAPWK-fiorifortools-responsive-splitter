...
// we need to look for the beginning of navigation
	 ...
	 *
	 ****************************************************************************************************************************/	
	autoCompleteUtilsLib.prototype.insertOMAlternatives = function(vocaRTServices, alts, name, prefix, comma) {
		...
		for (i = 0; i < alts.length; i++) {
			//JSON.stringify deals with unicode bad characters in different languages. The stringify adds " at the beginning of the word and at the end so we need to get rid of them by substring
			newAltName = JSON.stringify(alts[i].name);
	...
	autoCompleteUtilsLib.prototype.completeByValueListInLexerError = function(vocaRTServices, parser) {
		...
		if (parser.lexerErrorOccurred) {
			...
			if (altValueList.length > 0) {
				...
				var delimiter = "";
				valListSuggestions += this.insertOMAlternatives(vocaRTServices, altValueList, this.parsingOutputInfo.lastText, "", delimiter);
	...
	autoCompleteUtilsLib.prototype.completeNavSuggestions_dotMode = function(vocaRTServices, parser, prefix) {
		...
		if (navigationDetails.isValid === false) {
			if (navigationDetails.root.isValid === false) {
				...
				this.navigationCompletionInfo.navSuggestions += this.insertOMAlternatives(vocaRTServices, this.navigationCompletionInfo.navAlternatives, navigationDetails.root.name, prefix, this.navigationCompletionInfo.commaDelimiter);
				this.navigationCompletionInfo.navAlternatives = [];
			} else { // Example: player.
				...
				this.navigationCompletionInfo.navSuggestions += this.insertOMAlternatives(vocaRTServices, navigationDetails.attribute.altAttr.concat(this.navigationCompletionInfo.navAlternatives), navigationDetails.attribute.name, prefix, this.navigationCompletionInfo.commaDelimiter);
				this.navigationCompletionInfo.navAlternatives = [];
			}
		}
	};	
	...
	autoCompleteUtilsLib.prototype.completeNavSuggestions_termsMode = function(vocaRTServices, parser, prefix, lastToken) {
		...
		altNavigation = vocabularyUtilLib.getNavigationCompletion(this.navigationCompletionInfo.navigationTextInput,
				...
				parseModelLib.getModelManger().termMode);
		...
		if (this.navigationCompletionInfo.navAlternatives.length > 0) {
			this.navigationCompletionInfo.navSuggestions += this.insertOMAlternatives(vocaRTServices, this.navigationCompletionInfo.navAlternatives, this.navigationCompletionInfo.navigationTextInput, prefix, this.navigationCompletionInfo.commaDelimiter);
			this.navigationCompletionInfo.navAlternatives = [];
		}
