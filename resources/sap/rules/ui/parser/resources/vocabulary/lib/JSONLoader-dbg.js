...
sap.rules.ui.parser.resources.vocabulary.lib.JSONLoader.lib = (function() {
	...
	JSONLoaderLib.prototype.loadAll = function(rtAll, originalResourceContent, isPrivate, dbConn, resourceID, versionId, writeDependecies) {  //TBD: add the writeDependecies param to current hybrid caller !!!
		...
		function createObjectInfo(rtAll, objectModelContent, vocabularyID, vocaName, scope, isPrivate, dbConn) {
			...
			objectInfo = new vocaObjects.ObjectInfo(vocabularyID, vocaName, vocabularyConstants.OM_SOURCE, omID, omName, runtimeName, schema, scope, isPrivate, omDescription, runtimeType);
			//Add to the runtime cache
		...
		function createOutputRequiredParam(rtAll, paramContent, outputID, vocaName, dbConn) {
			...
			var outputRequiredParams, name = "", businessDataType = "", dataType = "", size = null, isCollection = null;
			...
			outputRequiredParams = new vocaObjects.OutputRequiredParams(outputID, name, dataType, size, businessDataType, isCollection, paramID);
			//Add to the runtime cache
		...
		function createOutputInfo(rtAll, outputContent, vocabularyID, vocaName, dbConn, scope, isPrivate, isValueListConverted) {
		...
		function createOutputInfos(rtAll, outputArrayContent, vocabularyID, vocaName, dbConn, scope, isPrivate, isValueListConverted) {
			...
			for (i = 0; i < outputArrayContent.length; ++i) {
				...
				createOutputInfo(rtAll, outputContent, vocabularyID, vocaName, dbConn, scope, isPrivate, isValueListConverted);
			}
		}
		...
		if (resourceContent.hasOwnProperty(vocabularyConstants.PROPERTY_NAME_OUTPUTS)) {
			...
			createOutputInfos(rtAll, resourceContent[vocabularyConstants.PROPERTY_NAME_OUTPUTS], vocabularyID, vocabularyFullName, dbConn, scope, isPrivate, isValueListConverted);
		}
