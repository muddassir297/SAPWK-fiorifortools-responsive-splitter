...
sap.zen.crosstab.rendering.ScrollManager = function (oCrosstab, oRenderEngine) {
	...
	function determineRenderedRowsInArea(oArea) {
		var iRowCnt = 0;
		if (oArea) {
			iRowCnt = $('#' + oArea.getId() + " > tbody").children("tr").length;
		}
		return iRowCnt;
	}
	...
	function displayHPopup (iScrollPos) {
		...
		if (!iHPopupWidth) {
			...
			if (sap.zen.crosstab.utils.Utils.isMainMode()) {
				iHPopupWidth = $('#' + oPopup.getContent().getId()).outerWidth();
			} else {
	...
	function displayVPopup (iScrollPos) {
		...
		if (!iVPopupWidth) {
			...
			if (sap.zen.crosstab.utils.Utils.isMainMode()) {
				iVPopupWidth = $('#' + oPopup.getContent().getId()).outerWidth();
			} else {
	...
	function determineRenderedColsInArea(oArea) {
		...
		var iColSpan = 1;
		var iColCnt = 0;
		$('#' + oArea.getId() + " > tbody").children("tr:last").children("td").each(function(index, domTd) {
