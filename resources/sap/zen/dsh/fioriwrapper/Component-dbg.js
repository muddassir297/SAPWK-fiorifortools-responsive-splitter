...
sap.zen.dsh.fioriwrapper.Component.prototype.createContent = function () {
	...
	if (this.getComponentData().startupParameters) {
		var paramValue = null;
		for (param in this.getComponentData().startupParameters) {
			if (this.getComponentData().startupParameters.hasOwnProperty(param) && param !== "newBW") {
				paramValue = this.getComponentData().startupParameters[param][0];
    ...
    oParseNavigationPromise.always(function(oStartupData, oURLParameters, sNavType){
