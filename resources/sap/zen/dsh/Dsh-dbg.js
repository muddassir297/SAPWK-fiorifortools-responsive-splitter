...
/*
 * Fix for message 1570012927, that occurs because smart business components update the viz
 * formatters. We wrap the format manage in code that prevents undefined being
 * returned if a design studio component calls format.
 *
 */
...
(function(global, $, undefined) {
	...
	function wrapVizFormatManagerFunctions(formatManager) {
		if (!formatManager) {
			return;
		}
		var oldFormat = formatManager.format;
		formatManager.format = function(value, type) {
			return type ? oldFormat.call(this, value, type) : value;
		};
	}

	if (global.require) {
		wrapVizFormatManagerFunctions(global.require("sap/viz/format/FormatManager"));
	}
})(window, jQuery);
