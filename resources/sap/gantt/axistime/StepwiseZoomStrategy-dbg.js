	...
	sap.gantt.axistime.StepwiseTimeLineOptions = {
		...
		"DayDate": {
			...
			smallInterval: {
				...
				span: 1,
				//e.g. Mon 22, Tue 23
				pattern: sap.ui.getCore().getConfiguration().getRTL() ? "d EEE" : "EEE d"
			}
		},
		"Date": {
		...
		if (iZoomLevel >= 0 && iZoomLevel !== this.getZoomLevel()) {
			...
			if (this._aZoomRate[iZoomLevel]) {
				...
				if (this._oTotalHorizonBeforeExtension) {
					if (this._oTotalHorizonBeforeExtension.getStartTime() !== oTotalHorizon.getStartTime()
							|| this._oTotalHorizonBeforeExtension.getEndTime() !== oTotalHorizon.getEndTime()) {
				...
				//Has to be executed after zoom level and rate are updated, otherwise the width of total horizon is incorrect.
				var iWidthOfTotalHorizon = this._getWidthOfTotalHorizon();
				if (iWidthOfTotalHorizon < this._nGanttVisibleWidth) {
				...
				var oNewVisibleHorizon = this.calVisibleHorizonByRate(this._aZoomRate[iZoomLevel], dMiddleOfVisibleHorizon);
