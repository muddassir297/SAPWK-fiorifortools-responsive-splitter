...
sap.ui.define([
	...
	"sap/gantt/eventHandler/MouseWheelHandler", "sap/gantt/axistime/AxisTimeStrategyBase", "sap/gantt/axistime/ProportionZoomStrategy", "sap/gantt/config/TimeHorizon", "sap/gantt/misc/ShapeSelectionModel",
	"sap/gantt/misc/ShapeManager", "sap/gantt/shape/Group",
	// 3rd party lib
	"sap/ui/thirdparty/d3"
		...
		TimeAxis, AxisTime, AxisOrdinal, Format, AutoScrollHandler, MouseWheelHandler, AxisTimeStrategyBase, ProportionZoomStrategy, TimeHorizon, ShapeSelectionModel,
		ShapeManager, Group ) {
	"use strict";
	...
	GanttChart.prototype.setSelectionMode = function (sSelectionMode) {
		...
		// passing down the selectionMode to ShapeSelectionModel
		this._oShapeSelection.setSelectionMode(sSelectionMode);
	...
	GanttChart.prototype._onMouseWheel = function(oEvent) {
		...
		if (bIsZoomTriggerred) {
			this.fireEvent("_mouseWheelZoom", {
				originEvent: oEvent
			});
		}
	};
	 ...
	 */
	GanttChart.prototype.syncMouseWheelZoom = function (oEvent){
	...
	GanttChart.prototype.selectShapes = function(aIds, bExclusive) {
		var aShapes = Utility.getShapeDatumById(aIds, this.getId());
		var aSelectableShapes = aShapes.filter(function(oItem) {
			return this.isShapeSelectable(oItem);
		}.bind(this));

		var bShapeSelectionChanged = this._oShapeSelection.selectShapes(aSelectableShapes, bExclusive);
	...
	GanttChart.prototype._selectionChange = function (oShapeData, bCtrl, bShift, origEvent) {
		...
		var oUpdated = this._oShapeSelection.changeShapeSelection(oShapeData, oRowInfo, bCtrl, this._bDragging);
	...
	GanttChart.prototype._handleAutoScroll = function (oEvent) {
		this._oAutoScrollHandler.autoScroll(this, oEvent);
	};
	...
	GanttChart.prototype._prepareDragging = function (oEvent) {
		if (oEvent.button == 0 ) {
			...
			if (sClassId && oShapeData && this.isShapeSelectable(oShapeData, sClassId)) {
				/**
	...
	GanttChart.prototype._handleSelectionChange = function (oEvent) {
		...
		if (oShapeData !== undefined && this.isShapeSelectable(oShapeData, sClassId)) {
			...
			oSelectionChange = this._selectionChange(oShapeData, oEvent.ctrlKey, oEvent.shiftKey, oEvent);
		} else {
			if (this.getSelectionMode() === sap.gantt.SelectionMode.MultiWithKeyboard) {
	...
	GanttChart.prototype._prepareRelationshipDataFromModel = function () {
		...
		Utility.generateUidForRelationship(this._aRelationships);
	};
