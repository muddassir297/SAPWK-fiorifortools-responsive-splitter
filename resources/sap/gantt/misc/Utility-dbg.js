...
sap.ui.define(["sap/gantt/misc/Format"], function (Format) {
	 ...
	 */
	Utility.generateUidForRelationship = function (aDataArray) {
		...
		for (var i = 0; i < aDataArray.length; i++) {
			if (aDataArray[i].id === undefined) {
				aDataArray[i].id = jQuery.sap.uid();
			}
			aDataArray[i].uid = sDummyPrefix + "|DATA:" + sShapeDataName + "[" + aDataArray[i].id + "]";
		}
	};
	...
	Utility.getDatumById = function(vId, sAttr, sContainer) {
		...
		aIds.forEach(function(id) {
			var selector = ["#", sContainer, " [data-", sAttr, "='", id, "']"].join("");
			$(selector).each(function(_, dom){
				datums.push(d3.select(dom).datum());
			});
		});
