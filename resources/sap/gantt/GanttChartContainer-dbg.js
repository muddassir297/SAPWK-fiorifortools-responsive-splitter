...
sap.ui.define([
	...
	GanttChartContainer.prototype.getCustomToolbarItems = function () {
		return this._oToolbar.getCustomToolbarItems();
	};
	...
	GanttChartContainer.prototype._insertGanttChart = function (oGanttChart, iIndex) {
		...
		this._adjustSplitterLayoutData(true, bReadConfig);
		...
		oGanttChart.attachEvent("_mouseWheelZoom", this._onGanttChartMouseWheelZoom, this);
	...
	GanttChartContainer.prototype._onGanttChartMouseWheelZoom = function(oEvent){
		//Currently this IF condition is a workaround: the mouse wheel zoom should be decoupled from the time scroll sync setting
		// TO DO: separate the logic of horizontal scroll sync and mouse wheel zoom sync
		if (!this.getEnableTimeScrollSync()) {
			//mouse wheel zoom should keep consistency between views even if the time scroll sync is disabled
			...
			for (var i = 0; i < aGanttCharts.length; i++){
				...
				//only zoom should be triggered
				aGanttCharts[i].syncMouseWheelZoom(oSourceEvent);
			}
		}
	};
	 ...
	 *
	 * @param {boolean} [bNeedReset] Set the layout size for gantt charts by calculating proper ratio size.
	 * @param {boolean} [bReadConfig] Apply configured layout to gantt charts and their selection panel.
	 * @private
	 */
	GanttChartContainer.prototype._adjustSplitterLayoutData = function (bNeedReset, bReadConfig) {
			...
			aContents.forEach(function(oContent, index){
				if (oContent.getLayoutData()) {
					if (index % 2 == 0) {
						...
						//when change hierarchy of the container, apply configuration
						if (bReadConfig) {
							sLayoutSize = that.getConfiguredGanttChartLayoutSize(that.getGanttCharts()[index]);
						} else if (bNeedReset) {//when switch orientation, just reset it
							sLayoutSize = iLayoutRatio + "%";
						}
						//finally, make sure the even content area has a layout size that is not "auto"
	...
	GanttChartContainer.prototype._removeGanttChart = function (vGanttChart) {
		...
		if (oGanttChart) {
			...
			this._adjustSplitterLayoutData(true);
			this._oSplitter.triggerResize(true);
			// detach events
	...
	GanttChartContainer.prototype._onGanttChartHSBScroll = function(oEvent){
		...
		for (var i = 0; i < aGanttCharts.length; i++){
			...
			//only scroll should be triggered
