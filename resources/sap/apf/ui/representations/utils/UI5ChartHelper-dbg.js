...
(function() {
	...
	sap.apf.ui.representations.utils.UI5ChartHelper = function(oApi, oParameters) {
		...
		var initExtendedDataResponse = function(aDataResponse, chartType) {
					...
					for(j = 0; j < Object.keys(this.displayNameLookup).length; j++) {
						...
						if (formattedFieldExists) {
							...
							if ((!textExists && sLabelDisplayOption === undefined) || sLabelDisplayOption === sap.apf.core.constants.representationMetadata.labelDisplayOptions.KEY) {
								...
								if (chartType === sap.apf.ui.utils.CONSTANTS.vizFrameChartTypes.LINE_CHART_WITH_TIME_AXIS){
									...
									this.extendedDataResponse[i][this.displayNameLookup[fieldName].DISPLAY_VALUE] = this.extendedDataResponse[i][fieldName];
								}
							}
		...
		this.init = function(aDataResponse, metadata, bIsGroupTypeChart, oFormatter, chartType) {
			this.metadata = metadata;
			this.formatter = oFormatter;
			initFieldsKeyLookup.bind(this)(metadata, aDataResponse);
			initExtendedDataResponse.bind(this)(aDataResponse, chartType);
		...
		this.getFilterFromSelection = function(aNewFilter) {
			...
			if (aNewFilter && aNewFilter.length > 0) {
				var aFiltersFromUI5 = self.filterValues.map(function(filterValue) {
					return filterValue[0];
				});
				var aUniqueFilters = jQuery.unique(aNewFilter.concat(aFiltersFromUI5));
				aUniqueFilters.forEach(function(newFilter) {
					if (self.filterValues.indexOf(newFilter) !== -1) {
						self.filterValues.push([ newFilter ]);
					}
				});
			}
			self.filterValues.forEach(function(filter) {
				reqFilterValues.push(filter[0]);
			});
