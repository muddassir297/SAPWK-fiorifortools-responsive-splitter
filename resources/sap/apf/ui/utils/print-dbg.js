...
sap.apf.ui.utils.Print = function(oInject) {
	...
	this.doPrint = function() {
		oCoreApi.getApplicationConfigProperties().done(function(applicationConfig) {
			...
			printModel.oChartSelectionPromise = jQuery.Deferred();
			if (printModel.nNoOfSteps === 0) {
				printModel.oChartSelectionPromise.resolve();
			}
	...
	function _printChainOfPromises(oPrintLayout, pTimer, buttonLayout) {
		...
		if (sap.ui.Device.browser.safari) {
			...
			window.setTimeout(function() { // Set Timeout to load the content on to dom
				...
				printModel.oChartSelectionPromise.then(function() {
					_processWindowPrint();
				});
				window.setTimeout(function() {
