	...
	sap.apf.core.Binding = function(oInject, oBindingConfig, oFactory, sRepresentationId) {
		...
		this.getFilter = function(oContextInfo) {
			...
			if (oSelectedRepresentation.getFilterMethodType() === methodTypes.filter) {
				...
				if (oStartFilter) {
					return afterGetFilter(oStartFilter.getInternalFilter().overwriteWith(oFilterFromRepresentation), oContextInfo);
				}
		...
		this.updateTreetable = function(controlObject, oModel, callbackAfterRequest, entityTypeMetadata) {
			this.getSelectedRepresentation().updateTreetable(controlObject, oModel, callbackAfterRequest, entityTypeMetadata);
		};
		/**
