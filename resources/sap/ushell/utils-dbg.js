...
sap.ui.define(function() {
    ...
    utils.handleTilesVisibility = function () {
        ...
        if (aTiles && aTiles.length) {
            ...
            aTiles.forEach(function (oTile) {
                var tileObject = utils.getTileObject(oTile);
                if (tileObject !== null) {
    ...
    utils.refreshTiles = function () {
        var aTiles = utils.getVisibleTiles(),
            launchPageService;
    ...
    utils.setTilesNoVisibility = function () {
        ...
        if ((typeof aTiles !== "undefined") && aTiles.length > 0) {
            ...
            aTiles.forEach(function (oTile) {
                launchPageService.setTileVisible(utils.getTileObject(oTile), false);
            });
            jQuery.sap.log.debug("Visible Tiles: " + aTiles.filter(function (oTile) {return oTile.isDisplayedInViewPort; }).length);
            jQuery.sap.log.debug("NonVisible Tiles: " + aTiles.filter(function (oTile) {return !oTile.isDisplayedInViewPort; }).length);
        }
    ...
    utils.calcVisibilityModes = function (oGroup, personalization) {
        var bIsVisibleInNormalMode = true,
            ...
            hasVisibleTiles = utils.groupHasVisibleTiles(oGroup.tiles, oGroup.links);
     ...
     * @param {number} nIndexOfElement
     *  The index of the element which needs to be moved
     * @param {number} nNewIdx
     *  The index where to element should be moved to
     * @returns {array|undefined}
     *  The array after the move of the element. Returns 'undefined' if the operation is not possible
     *
     * @since 1.39.0
     * @public
     */
    utils.moveElementInsideOfAnArray = function (aArray, nIndexOfElement, nNewIdx) {
        var oElementToMove, nIdx;
        if (!aArray || nIndexOfElement < 0 || nNewIdx < 0 ||
                typeof nIndexOfElement !== "number" || typeof nNewIdx !== "number" ||
                nIndexOfElement > aArray.length || nNewIdx > aArray.length) {
            return undefined;
        } else if (nIndexOfElement === nNewIdx){
            return aArray;
        }
        oElementToMove = aArray[nIndexOfElement];
        //check if the element should be moved to higher index or lower index
        if (nIndexOfElement < nNewIdx) {
            for (nIdx = nIndexOfElement; nIdx < nNewIdx; nIdx++) {
                aArray[nIdx] = aArray[nIdx + 1];
            }
        } else if (nIndexOfElement > nNewIdx) {
            for (nIdx = nIndexOfElement; nIdx > nNewIdx; nIdx--) {
                aArray[nIdx] = aArray[nIdx - 1];
