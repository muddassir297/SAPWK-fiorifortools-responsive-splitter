...
sap.ui.define(['sap/m/Text',
    ...
    var LaunchPageAdapter = function (oUnused, sParameter, oAdapterConfiguration) {
        ...
        this.isLinkPersonalizationSupported = function (oTile) {
            if (oTile) {
                return  (window.location.search.indexOf("new_links_container=true") !== -1) && oTile.isLinkPersonalizationSupported;
            }
            return (window.location.search.indexOf("new_links_container=true") !== -1);
        };
                    ...
                    try {
                        if (url){
                            ...
                            this._createTileInstance(oTileData, sTileType).done(function (oTile) {
                                ...
                                if (bIsLink && window.location.search.indexOf("new_links_container=true") === -1) {
                                    oTileUI.setHref(url);
                                    if (url[0] !== '#'){
            ...
            switch (sTileType) {
                ...
                case 'Link':
                    if (window.location.search.indexOf("new_links_container=true") === -1) {
                        oTileUI = new Link ({
                            text: oTileData.properties.title
                        });
                    } else {
                        oTileUI = new GenericTile ({
        ...
        this._genericTilePressHandler = function (oTileData, oEvent) {
            if (window.location.search.indexOf("new_links_container=true") === -1 || (oEvent.getSource().getScope && oEvent.getSource().getScope() === "Display")) {
