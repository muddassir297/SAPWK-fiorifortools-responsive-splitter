        ...
        getPieChartPlaceholder2: function() {
            ...
            oChart1.addEventDelegate({
                onAfterRendering: function(oEvent) {
                    ...
                    //$('.largeChart2piechart.sapVizFrame').css('margin-left', '-50px');
                    // var $legendItem = $(".v-legend-item").last();
                    //$legendItem.unbind();
                    //$legendItem.outerHTML = $legendItem.outerHTML;
                    //$legendItem.off();
                    //var n = 1;
            ...
            oChart1.attachRenderComplete(function(oEvent) {
                ...
                //var percentageMissingInBigPie = aSearchFacetItems[0].percentageMissingInBigPie;
                //var newItemLabel = sap.ushell.resources.i18n.getText("facetPieChartOverflowText2", [percentageMissingInBigPie, 9]);

                //try add new wedge to pie - fail
                //FlattenedDataset manages the 'data' aggregation only via data binding. 
                //The method 'insertData' therefore cannot be used programmatically! - 
                /*

                var averageSliceValue = 1;
                var newItem = aSearchFacetItems[0].clone([true, true]);
                ...
                oChart1.getDataset().insertData(newItem, 0);
				


                var oDescription = $(".ui5-viz-controls-viz-description");
                oDescription.css("display", "block");
                oDescription[0].firstChild.innerHTML = newItemLabel;
...
*/
                ...
                /*
                                $wedgeElement.attr("fill-opacity", "0");
                                $wedgeElement.attr("stroke-opacity", "1");
                                $wedgeElement.attr("stroke", "black");
                                $wedgeElement.attr("stroke-width", "0.5");
            ...
            /*
            var showTooltipFunction = function(options) {
                if (!this._rootNode) {
                ...
                if (options.data && options.data.length) {
                    for (var i = -1; ++i < options.data.length;) {
                        ...
                        if (item.name == "Label") {
                            ...
                            tempString = "New-Profit";
                        } else {
                            tempString = item.name;
                        }
                        if (item.name !== "_context_row_number")
                            itemNode = $("<div class='datum'><span class='key'>" + tempString + "</span><span class='value'>" + item.value + "</span></div>").appendTo(dataNode);
                ...
                this._rootNode.css({
                    ...
                    "left": options.position.x + 16,
                    //"bottom": $(document).height() / 2 - options.position.y + 170 //let tooltip showing above mouse point
                    "top": options.position.y + 16
                });

            };
            var hideTooltipFunction = function(mode) {
			...
			*/
            oChart1.setVizProperties({
                ...
                interaction: {
                    ...
                    decorations: [{
                        name: "showDetail",
                        fn: showTooltipFunction
                    }, {
                        name: "hideDetail",
                        fn: hideTooltipFunction
                    }]
					*/
                }
            });
