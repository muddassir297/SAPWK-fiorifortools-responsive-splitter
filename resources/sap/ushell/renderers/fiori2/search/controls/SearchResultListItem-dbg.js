...
(function() {
    ...
    sap.ui.core.Control.extend("sap.ushell.renderers.fiori2.search.controls.SearchResultListItem", {
        ...
        onAfterRendering: function() {
            ...
            $that.find('.sapUshellSearchResultListItem-MightOverflow').on('mouseenter', function() {
                var $this = $(this);
                var title = $this.attr('title');
                var tooltip;
                var tooltippedby = $this.attr("data-tooltippedby");
                if (tooltippedby) {
                    var tooltippedbyElement = $that.find("#" + tooltippedby);
                    tooltip = tooltippedbyElement.text();
                }
                if ((this.offsetWidth < this.scrollWidth || this.offsetHeight < this.scrollHeight || tooltip) && !$this.attr('title')) {
                    if (!tooltip) {
                        tooltip = $this.text();
                    }
            ...
            return function() {
                if (navigationObject) {
                    try {
                        ...
                        model.eventLogger.logEvent({
                            type: model.eventLogger.ITEM_NAVIGATE,
                            targetUrl: navigationObject.getHref()
                        });
                    } catch (e) { /* eslint no-empty:0 */ }
