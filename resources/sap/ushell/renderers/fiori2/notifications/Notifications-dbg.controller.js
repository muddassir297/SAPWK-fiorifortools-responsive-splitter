...
sap.ui.define(['sap/ushell/utils'],
    ...
    sap.ui.controller("sap.ushell.renderers.fiori2.notifications.Notifications", {
        ...
        onInit: function () {
            ...
            oInitialModelStructure[this.oSortingType.NOTIFICATIONS_BY_TYPE_DESCENDING] = this.getInitialSortingModelStructure();
            ...
            this.sCurrentTypeExpanded = undefined;

            // Initializing the model with a branch for each sorting type
            this.getView().setModel(new sap.ui.model.json.JSONModel(oInitialModelStructure));
        ...
        getNextBufferForType: function () {
            var selectedTypeId = this.sCurrentTypeExpanded,
            ...
            // Fetch a buffer of notification items from notification service
            oPromise = this.oNotificationsService.getNotificationsBufferInGroup(selectedTypeId, iNumberOfItemsInModel, this.getBufferSize());

            oPromise.done(function (oResult) {
                this.addTypeBufferToModel(selectedTypeId, oResult);
            }.bind(this));
         ...
         * Gets a buffer of notification items from notification service, according to the current sorting type
         */
        getTypesHeaders: function () {
            var aCurrentItems = this.getItemsFromModel(this.oSortingType.NOTIFICATIONS_BY_TYPE_DESCENDING),
                iNumberOfItemsInModel,
                oPromise,
                bHasMoreItems = this.getView().getModel().getProperty("/" + this.oSortingType.NOTIFICATIONS_BY_TYPE_DESCENDING + "/hasMoreItems");
            ...
            // Adding Busy Indicator if its the first loading
            if (iNumberOfItemsInModel === 0) {
                this.addBusyIndicatorToTabFilter(true);
            }
            this.getView().getModel().setProperty("/" + this.oSortingType.NOTIFICATIONS_BY_TYPE_DESCENDING + "/inUpdate", true);

            // Fetch a buffer of notification items from notification service
            oPromise = this.oNotificationsService.getNotificationsGroupHeaders();

            oPromise.done(function (oResult) {
                this.addTypeHeadersToModel(oResult);
            }.bind(this));
         ...
         * The hasMoreItems flag indicates whether the number of returned items is smaller than the size of the requested buffer,
         *  if so (i.e. oResultObj.value.length < getBufferSize()) then there are no more items in the beckend for this sorting type.
         *
         ...
         */
        addTypeBufferToModel: function (sTypeId, oResult) {
            var oGroup = this.getGroupFromModel(sTypeId),
                oGroupIndexIndexInModel = this.getGroupIndexFromModel(sTypeId),
                aGroupHeaders = this.getView().getModel().getProperty("/" + this.oSortingType.NOTIFICATIONS_BY_TYPE_DESCENDING + "/aGroupHeaders/"),
            // oResultObj,
                mergedArrays;
            ...
            // If the number of returned items is smaller than the number that was requested -
            // it means that there is no more data (i.e. notification items) in the backend that needs to be fetched for this sorting type
            // if (oResultObj.value.length < this.getBufferSize()) {
            if (oResult.length < this.getBufferSize()) {
            ...
            aGroupHeaders[oGroupIndexIndexInModel].aNotifications = mergedArrays;

            this.getView().getModel().setProperty("/" + this.oSortingType.NOTIFICATIONS_BY_TYPE_DESCENDING + "/aGroupHeaders/", aGroupHeaders);
            this.getView().getModel().setProperty("/" + this.oSortingType.NOTIFICATIONS_BY_TYPE_DESCENDING + "/inUpdate", false);
        ...
        notificationsUpdateCallback: function (oDependenciesDeferred) {
            ...
            if (this.sCurrentSorting === this.oSortingType.NOTIFICATIONS_BY_TYPE_DESCENDING) {
                this.oldNotificationsUpdateCallbackForType();
        ...
        getNotificationsByTypeWithGroupHeaders: function () {
            ...
            oPromise.fail(function (data) {
                jQuery.sap.log.error("Notifications control - call to notificationsService.getNotificationsByTypeWithGroupHeaders failed: ",
                    data,
                    "sap.ushell.renderers.fiori2.notifications.Notifications");
        ...
        addNotificationToModel: function (oNotification, index) {
            var oModel = this.getView().getModel(),
                ...
                oTabBarSelectedSort = this.getView().oIconTabBar.getSelectedKey();
            notifications.splice(index, 0, oNotification);
            oModel.setProperty("/" + this.sCurrentSorting + "/aNotifications", notifications);
            if (oTabBarSelectedSort === "sapUshellNotificationIconTabByType") {
        ...
        removeNotificationFromModel: function (notificationId) {
            ...
            if (oTabBarSelectedSort === "sapUshellNotificationIconTabByDate" || oTabBarSelectedSort === "sapUshellNotificationIconTabByPrio") {
            ...
            for (index = 0; index < aGroups.length; index++) {
                notifications = aGroups[index].notifications;
                if (notifications.length === 1 && notifications[0].Id === notificationId) {
                    ...
                    aGroups[index].notifications = notifications;
                }
            }
            oModel.setProperty("/aNotificationsByType", aGroups);
            return oRemovedNotification;
        ...
        getIndexInModelByItemId: function (sNotificationId) {
            var aNotifications = this.getView().getModel().getProperty("/" + this.sCurrentSorting + "/aNotifications"),
                index;
        ...
        cleanModel: function () {
            ...
            jQuery.each(oSortingTypesArray, function (index, item) {
                if (index !== that.sCurrentSorting && index !== "aNotificationsByType" ) {
        ...
        getGroupFromModel: function (sTypeHeader) {
            var aGroupHeaders = this.getView().getModel().getProperty("/" + this.oSortingType.NOTIFICATIONS_BY_TYPE_DESCENDING + "/aGroupHeaders/"),
                oGroup;
            aGroupHeaders.every(function (group, index) {
        ...
        triggerRetrieveMoreDataForGroupNotifications: function () {
            if (!this.getModel().getProperty("/" + this.oSortingType.NOTIFICATIONS_BY_TYPE_DESCENDING + "/inUpdate")) {
                var notificationsInModel = this.getItemsOfTypeFromModel(this.sCurrentTypeExpanded),
                    numberOfNotificationsInModel = notificationsInModel ? notificationsInModel.length : 0,
                    bufferSize = numberOfNotificationsInModel ? this.getBufferSize() : 0,
        ...
        collapseOtherGroups: function (groupId) {
            var listItems = this.getView().oNotificationsListType.getItems();
            listItems.forEach(function (item, index) {
                if (item.getId() === groupId && item.getCollapsed()) {
                    item.setCollapsed(false);
                } else if (item.getId() !== groupId && !item.getCollapsed()) {
                    item.setCollapsed(true);
                }
            });
        },
        oldNotificationsUpdateCallbackForType: function () {
            var that = this;
