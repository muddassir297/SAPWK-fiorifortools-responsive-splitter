    ...
    CatalogsContainer.prototype.setCategoryAllocateTiles = function (nAllocatedUnits) {
        ...
        this.filters.pagination = function (entry) {
            ...
            if (this.nAllocatedUnits > 0) {
                //first process the leftover for the lastPagedCatalog.
                if (this.oLastPaggedCatalog) {
                    //Validate that we have no appBoxesContainer we can add.
                    this.oLastPaggedCatalog.handleElements("appBoxesContainer");
                    //Validate that we have no customTilesContainer we can add.
                    this.oLastPaggedCatalog.handleElements("customTilesContainer");
                }
            }

            //this catalog is not in the paggination yet.
            if (!this.catalogPagination[sPath]) {
                indexEntry = this.indexingMaps.onScreenPathIndexMap[sPath];
                //do we have more allocated units.
                if (this.nAllocatedUnits > 0) {
                    //Then is we still have place add the new group and process it.
                    ...
                    //Update this filter as a catalog that is in the paggination.
                    this.catalogPagination[sPath] = true;
                } else {
                    //If we do not have allocated units for this catalog the its visibility will be false.
                    indexEntry.isVisible = false;
                    this.catalogPagination[sPath] = false;
                }
            }
    ...
    CatalogsContainer.prototype.addNewItem = function (elementToDisplay, sName) {
        ...
        oNewCatalog.newElementCreated = function (sElementType, oCatalog) {
            ...
            if (this.nAllocatedUnits <= 0) {
                ...
                //mak this catalog as not complite, next allocation start with compliting this catalog.
                this.oLastPaggedCatalog = oCatalog;
            }
        }.bind(this);
