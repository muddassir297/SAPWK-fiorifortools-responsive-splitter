...
function () {
    ...
    PersonalizationProcessor.prototype
        ...
        if (!oPersonalizationDelta || jQuery.isEmptyObject(oPersonalizationDelta)) {
        ...
        if (oPersonalizationDelta.removedGroups && jQuery.isArray(oPersonalizationDelta.removedGroups)) {
    ...
    PersonalizationProcessor.prototype._applyAddGroups = function (
        ...
        if (jQuery.isArray(oPersonalizationDelta.groupOrder)) {
        ...
        if (
            jQuery.isArray(oPersonalizationDelta.groupOrder)
    ...
    PersonalizationProcessor.prototype
        ._applyTileSettings = function (oSite, oPersonalizationDelta) {
            ...
            aGroups.some(function (oGroup) {
                oGroup.payload.tiles.some(function (oTile) {
        ...
        .extractPersonalization = function (oPersonalizedSite, oOriginalSite) {
            ...
            if (oPersonalizedSite && oPersonalizedSite.groups && oOriginalSite && oOriginalSite.groups) {
                ...
                if (oExtractHelper.oPersonalizationDelta.movedTiles
                    && jQuery.isEmptyObject(oExtractHelper.oPersonalizationDelta.movedTiles)
                ) {
        ...
        if (oPersonalizedSite && oPersonalizedSite.groups && oPersonalizationDelta && sOriginalGroupId) {
            if (!oPersonalizedSite.groups[sOriginalGroupId]) {
                ...
                if (oPersonalizedSite.site
                        ...
                        && oPersonalizedSite.site.payload.groupsOrder
                        && jQuery.isArray(oPersonalizedSite.site.payload.groupsOrder)
                ) {
    ...
    PersonalizationProcessor.prototype._checkMoveGroup
        ...
        if (oPersonalizedSite.site
                ...
                && jQuery.isArray(oOriginalSite.site.payload.groupsOrder)
        ) {
    ...
    PersonalizationProcessor.prototype._applyMoveGroup = function (
        ...
        if (jQuery.isArray(oPersonalizationDelta.groupOrder)) {
            oOriginalSite.site = oOriginalSite.site || {};
            oOriginalSite.site.payload = oOriginalSite.site.payload || {};
            if (!jQuery.isArray(oOriginalSite.site.payload.groupsOrder)) {
        ...
        if (oSiteObject && oSiteObject.groups) {
            if (Object.keys(oSiteObject.groups).length > 0) {
                ...
                Object.keys(oSiteObject.groups).forEach(function (sGroupId) {
                    ...
                    if (oGroup.payload && jQuery.isArray(oGroup.payload.tiles)) {
                        aGroupTiles = oGroup.payload.tiles;
                        aGroupTiles.forEach(function (oItem, iIndex) {
                            var sItemId;

                            if (oItem.id) {
                                sItemId = oItem.id;
                                oResult[sGroupId][sItemId] = {};
                                oResult[sGroupId][sItemId].iIndex = iIndex;
                                oResult[sGroupId][sItemId].oItem = oItem;
    ...
    PersonalizationProcessor.prototype._getExtractHelperObject
        ...
        oResult = {};
        oResult.oHashedItemsOriginal = this._getHashedItemsFromSite(oOriginalSite);
    ...
    PersonalizationProcessor.prototype._extractFromOriginalSiteOneGroup
        ...
        // Define inner helper method for creating a "removed entry" in oExtractHelper.oPersonalizationDelta.movedTiles:
        ...
        if (oExtractHelper
                ...
                && jQuery.isArray(oOriginalSite.groups[sGroupId].payload[sItemType])
        ) {
            aOriginalItems = oOriginalSite.groups[sGroupId].payload[sItemType];
            oExtractHelper.oPersonalizationDelta.movedTiles = oExtractHelper.oPersonalizationDelta.movedTiles
                    ? oExtractHelper.oPersonalizationDelta.movedTiles : {};
            oMovedTiles = oExtractHelper.oPersonalizationDelta.movedTiles;
            if (oExtractHelper.oHashedItemsPersonalized[sGroupId]) {
                ...
                aOriginalItems.forEach(function (oItem) {
                    if ( !oExtractHelper.oHashedItemsPersonalized[sGroupId][oItem.id]) {
    ...
    PersonalizationProcessor.prototype._extractFromPersonalizedSiteOneGroup
        ...
        if (oExtractHelper
                && oExtractHelper.oHashedItemsOriginal
                ...
                && jQuery.isArray(oPersonalizedSite.groups[sGroupId].payload[sItemType])
        ) {
            ...
            aPersonalizedItems.forEach(function (oItem, iIndex) {
                ...
                if ( oExtractHelper.oHashedItemsOriginal[sGroupId]
                        && oExtractHelper.oHashedItemsOriginal[sGroupId][oItem.id]
                ) {
                    ...
                    if (oExtractHelper.oHashedItemsOriginal[sGroupId][oItem.id].iIndex !== iIndex) {
            ...
            // Save tileOrder, if needed:
            if (bTileOrderChanged) {
                // Move temporary tileOrder to persistence location (e.g. personalized delta):
    ...
    PersonalizationProcessor.prototype._cleanupRemovedGroups
        ...
        if (oPersonalizationDelta.removedGroups
                && oPersonalizationDelta
                && jQuery.isArray(oPersonalizationDelta.removedGroups)
            ...
            oPersonalizationDelta.removedGroups.forEach(function (sGroupId) {
                if (oOriginalSite
                        ...
                        && oOriginalSite.groups[sGroupId]
                        && oOriginalSite.groups[sGroupId].payload
                        && jQuery.isArray(oOriginalSite.groups[sGroupId].payload.tiles)
                ) {
                    aTiles = oOriginalSite.groups[sGroupId].payload.tiles;
                    aTiles.forEach(function (oTile) {
                        if (oPersonalizationDelta.movedTiles) {
                            oMoveTileEntry = oPersonalizationDelta.movedTiles[oTile.id];
                            if (oMoveTileEntry
                                    && oMoveTileEntry.fromGroup
                                    && (oMoveTileEntry.fromGroup === sGroupId)
                                    && !oMoveTileEntry.toGroup
                            ) {
                                delete (oPersonalizationDelta.movedTiles[oTile.id]);
                            }
                        }
                    });
                }
    ...
    PersonalizationProcessor.prototype
        ...
        if (!jQuery.isArray(aOriginalSiteItems)
                || !jQuery.isArray(aOriginalSiteItems)
    ...
    PersonalizationProcessor.prototype
        ...
        if (oDeltaGroup.payload
                && jQuery.isArray(oDeltaGroup.payload[sItemOrder])
                && (oDeltaGroup.payload[sItemOrder].length > 0)
        ) {
            if (oOriginalGroup.payload
                    && jQuery.isArray(oOriginalGroup.payload[sItemType])
    ...
    PersonalizationProcessor.prototype
        ...
        function fInsertItem (oCDMSiteItem, sGroupId) {
            ...
            if (oOriginalSite.groups[sGroupId]
                    ...
                    && jQuery.isArray(oPersonalizationDelta.groups[sGroupId].payload[sItemOrder])
                    && (oPersonalizationDelta.groups[sGroupId].payload[sItemOrder].length > 0)
            ) {
                if (!jQuery.isArray(oOriginalSite.groups[sGroupId].payload[sItemType])) {
        ...
        if (oPersonalizationDelta[sTypeKey]) {
            Object.keys(oPersonalizationDelta[sTypeKey]).forEach(function (sItemId) {
                ...
                if (oDeltaItem.fromGroup) {
                    if (oDeltaItem.toGroup) {
                        ...
                        if (oDeltaItem.fromGroup !== oDeltaItem.toGroup) {
                            ...
                            if (oOriginalSite.groups[oDeltaItem.fromGroup]
                                    ...
                                    && jQuery.isArray(oOriginalSite.groups[oDeltaItem.fromGroup].payload[sItemType])
                            ) {
                        ...
                        // Surely a removed item.
                        // Unfortunately we don't have position of removed item, so we have to iterate:
                        if (oOriginalSite.groups[oDeltaItem.fromGroup]
                                && oOriginalSite.groups[oDeltaItem.fromGroup].payload
                                && jQuery.isArray(oOriginalSite.groups[oDeltaItem.fromGroup].payload[sItemType])
                        ) {
                    ...
                    if (oDeltaItem.item && !jQuery.isEmptyObject(oDeltaItem.item)) {
