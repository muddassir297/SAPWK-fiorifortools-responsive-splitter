    ...
    function Notifications(oContainerInterface, sParameters, oServiceConfiguration) {
        ...
        */
        ...
        this._setNotificationsAsSeen = function () {
            var sResetBadgeNumberUrl = this._getRequestURI(oOperationEnum.RESET_BADGE_NUMBER),
                oRequestObject = {
        ...
        this._getRequestURI = function (oRequiredURI, oArgs) {
            ...
            switch (oRequiredURI) {
                ...
                case oOperationEnum.NOTIFICATIONS_IN_GROUP:
                    if (aRequestURIs.getNotificationsInGroup.basic === undefined) {
                        aRequestURIs.getNotificationsInGroup.basic = oServiceConfig.serviceUrl + "/Notifications?$expand=Actions,NavigationTargetParams&$orderby=CreatedAt%20desc&$filter=IsGroupHeader%20eq%20false&ParentId%20eq%20" + oArgs.group + "$skip=" + oArgs.skip + "&$top=" + oArgs.top;
                    }
                    if (this._isIntentBasedConsumption()) {
                        if (aRequestURIs.getNotificationsInGroup.byIntents === undefined) {
                            aRequestURIs.getNotificationsInGroup.byIntents = aRequestURIs.getNotificationsInGroup.basic.concat("&intents%20eq%20" + sEncodedConsumedIntents);
                        }
                        return aRequestURIs.getNotificationsInGroup.byIntents;
                    }
                    return aRequestURIs.getNotificationsInGroup.basic;
           ...
           var sNotificationId,
            ...
            if (oNotificationData !== undefined) {
                ...
                // Background use-case (oNotificationData.additionalData is defined and equals "false")
                    ...
                    // The navigation parameters should be an array
                    // (if it is a string - then it is converted to an array
        ...
        */
        ...
        this._updateCSRF = function (oResponseData) {
            ...
            if (!this._getHeaderXcsrfToken()) {
                sHeaderXcsrfToken = oResponseData.headers["x-csrf-token"] || oResponseData.headers["X-CSRF-Token"];
            }
        ...
        this._userSettingInitialization = function () {
            ...
            oMobileSettingsPromise.done(function (oResult) {
                // Push to Mobile validaiton returned
                bNotificationSettingsMobileSupport = oResult ? JSON.parse(oResult).IsActive : false;
                oSettingsStatus.mobileAvailable = bNotificationSettingsMobileSupport;
            });

            // If notification setting is not available, the fedault user choise for preview enebling is set to true,
            // otherwise the user can't see preview at all
            oMobileSettingsPromise.fail(function () {
